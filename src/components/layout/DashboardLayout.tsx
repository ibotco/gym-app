...
import {
  ...
  BarChart3, Bell, Settings, Shield, ShieldCheck, ClipboardList, ClipboardCheck, Dumbbell, MessageSquare, Mail, MessageSquareText,
...
import { branchDepth, branchesTreeOrder, visibleBranches, visibleCompanies, userCompanyId } from '../../lib/accessScope'
...
const NAV: NavEntry[] = [
  ...
  {
    kind: 'group', key: 'nav.organizations', icon: Building2, roles: ['super_admin', 'gym_manager', 'company_admin', 'head_office'],
    children: [
      { to: '/admin/companies', key: 'nav.companies', icon: Building2, roles: ['super_admin', 'gym_manager', 'company_admin', 'head_office'] },
      { to: '/admin/branches', key: 'nav.branches', icon: Building2, roles: ['super_admin', 'gym_manager', 'company_admin'] },
      { to: '/admin/cost-centers', key: 'nav.costCenters', icon: Wallet, roles: ['super_admin', 'gym_manager', 'company_admin'] },
    ],
  },
  {
  ...
  {
    kind: 'group', key: 'nav.userManagement', icon: UserCog, roles: ['super_admin', 'gym_manager', 'company_admin'],
    children: [
      { to: '/admin/users', key: 'nav.users', icon: UserCog, roles: ['super_admin', 'gym_manager', 'company_admin'] },
      { to: '/admin/roles', key: 'nav.roles', icon: ShieldCheck, roles: ['super_admin', 'gym_manager', 'company_admin'] },
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { to: '/admin/inventory/stock/count', key: 'nav.invStockCount', icon: ClipboardCheck, roles: ['super_admin', 'gym_manager', 'accountant'] },
  ...
  },
  {
    ...
    children: [
      ...
      { to: '/admin/pos2', key: 'nav.pos2', icon: ShoppingCart, roles: ['super_admin', 'gym_manager', 'staff'] },
  ...
  },
  {
    ...
    children: [
      { to: '/admin/purchase-requisitions', key: 'nav.requisitions', icon: ClipboardList, roles: ['super_admin', 'gym_manager', 'staff'] },
      { to: '/admin/procurement-orders', key: 'nav.procurementOrders', icon: ShoppingCart, roles: ['super_admin', 'gym_manager', 'staff'] },
      { to: '/admin/goods-receipts', key: 'nav.goodsReceipts', icon: PackageCheck, roles: ['super_admin', 'gym_manager', 'staff'] },
      { to: '/admin/supplier-invoices', key: 'nav.supplierInvoices', icon: Receipt, roles: ['super_admin', 'gym_manager', 'staff'] },
      { to: '/admin/supplier-payments', key: 'nav.supplierPayments', icon: Banknote, roles: ['super_admin', 'gym_manager', 'staff'] },
      { to: '/admin/procurement-returns', key: 'nav.procurementReturns', icon: Undo2, roles: ['super_admin', 'gym_manager', 'staff'] },
      { to: '/admin/procurement-reports', key: 'nav.procurementReports', icon: BarChart3, roles: ['super_admin', 'gym_manager', 'staff'] },
    ],
  },
  {
  ...
  {
    kind: 'group', key: 'nav.communications', icon: MessageSquare, roles: ['super_admin', 'gym_manager'],
    children: [
      { to: '/admin/cms/events', key: 'nav.events', icon: CalendarDays, roles: ['super_admin', 'gym_manager'] },
      { to: '/admin/notifications', key: 'nav.notifications', icon: Bell, roles: ['super_admin', 'gym_manager'] },
      { to: '/admin/email-templates', key: 'nav.emailTemplates', icon: Mail, roles: ['super_admin', 'gym_manager'] },
      { to: '/admin/sms-templates', key: 'nav.smsTemplates', icon: MessageSquareText, roles: ['super_admin', 'gym_manager'] },
    ],
  },
...
// Company Admin inherits every navigation area Super Admin can see, except
// the platform-wide System Settings (super-admin only by design).
for (const entry of NAV as Array<NavEntry & { children?: GroupChild[] }>) {
  const grant = (node: { roles: Role[] }) => {
    if (node.roles.includes('super_admin') && !node.roles.includes('company_admin')) node.roles.push('company_admin')
  }
  grant(entry)
  if (entry.kind === 'group') {
    const walk = (nodes: GroupChild[]) => {
      for (const n of nodes) {
        grant(n as { roles: Role[] })
...
const systemSettingsEntry = NAV.flatMap((e) => (e.kind === 'group' ? e.children : [])).find(
  (c): c is Item => 'to' in c && c.to === '/admin/settings/system',
)
if (systemSettingsEntry) systemSettingsEntry.roles = ['super_admin']

export function DashboardLayout() {
  const { user, logout, impersonate, hasRole } = useAuth()
  const { isDark, toggle } = useTheme()
  const {
    users, members, suppliers, customers, staff, trainers, roles, notifications, markAllNotifRead, modules, sidebarOrder, company,
    companies, branches, activeCompany, activeCompanyId, activeBranchId,
  ...
  /**
   * Custom header/footer chrome: force readable text & border on top of the
   * custom colour. The `dark` class alone only flips `dark:` utilities — the
   * branch/company names inherit `color` from the body, which stays near-black
   * in light mode and would be invisible on a dark (e.g. Enterprise Navy) bar.
   */
  const headerChromeStyle: CSSProperties | undefined = headerColor
    ? {
        backgroundColor: headerColor,
        color: headerChromeDark ? '#f4f4f5' : '#18181b',
        borderColor: headerChromeDark ? 'rgb(255 255 255 / 0.14)' : 'rgb(0 0 0 / 0.08)',
      }
    : undefined
  // Multi-company / multi-branch context.
  const currentRoleDefinition = user ? roles.find((role) => role.id === user.role && !role.builtin) : undefined
  const customRoleIsCompanyScoped = Boolean(currentRoleDefinition && currentRoleDefinition.portal === 'admin' && (currentRoleDefinition.companyId || user?.companyId))
  const customRoleIsBranchScoped = Boolean(currentRoleDefinition && currentRoleDefinition.portal !== 'admin' && (currentRoleDefinition.companyId || user?.companyId))
  ...
  const isBranchScoped = user?.role === 'branch_admin' || customRoleIsBranchScoped
  const isCompanyScoped = user?.role === 'company_admin' || user?.role === 'head_office' || customRoleIsCompanyScoped
  const isAssignedBranchUser = user?.role === 'staff' || user?.role === 'trainer' || user?.role === 'member' || user?.role === 'supplier' || user?.role === 'customer' || customRoleIsBranchScoped
  const assignedBranchId = user?.branchId
    || (user ? members.find((record) => record.userId === user.id)?.branchId : undefined)
    || (user ? suppliers.find((record) => record.userId === user.id)?.branchId : undefined)
    || (user ? customers.find((record) => record.userId === user.id)?.branchId : undefined)
    || (user ? staff.find((record) => record.userId === user.id)?.branchId : undefined)
    || (user ? trainers.find((record) => record.userId === user.id)?.branchId : undefined)
  const assignedBranch = branches.find((branch) => branch.id === assignedBranchId)
  const tenantCompanyId = user && (isBranchScoped || isCompanyScoped) ? userCompanyId(user, branches) : activeCompanyId
  const tenantCompanies = useMemo(
    () => visibleCompanies(user, companies, branches),
    [branches, companies, user],
  )
  const scopedCompany = tenantCompanies.find((candidate) => candidate.id === tenantCompanyId) || tenantCompanies[0] || activeCompany
  const companyBranches = useMemo(
    () => visibleBranches(user, branches, scopedCompany?.id || tenantCompanyId),
    [branches, scopedCompany?.id, tenantCompanyId, user],
  )
  const selectedBranch = companyBranches.find((branch) => branch.id === activeBranchId) || companyBranches[0]

  // Keep persisted company/branch context inside the signed-in user's scope.
  useEffect(() => {
    if ((isBranchScoped || isCompanyScoped) && scopedCompany && activeCompanyId !== scopedCompany.id) {
      setActiveCompany(scopedCompany.id)
    }
    if (isBranchScoped && user?.branchId && activeBranchId !== user.branchId) {
      setActiveBranch(user.branchId)
    }
  }, [activeBranchId, activeCompanyId, isBranchScoped, isCompanyScoped, scopedCompany, setActiveBranch, setActiveCompany, user?.branchId])

  const advanceMode = productMode === 'advance'
  const showOrgContext = advanceMode || ['super_admin', 'company_admin', 'branch_admin', 'head_office'].includes(user?.role || '')
  const { t } = useI18n()
  ...
  const items = useMemo(() => {
    ...
    const customDefinition = user ? roles.find((role) => role.id === user.role && !role.builtin) : undefined
    // Custom admin roles use their actual permission list rather than being
    // treated as a full Gym Manager. This keeps the sidebar useful after a
    // Company Admin assigns a tenant role while preserving least privilege.
    const CUSTOM_NAV_PERMISSIONS: Record<string, string[]> = {
      'nav.overview': ['dashboard.view'],
      'nav.organizations': ['companies.view', 'branches.view'],
      'nav.companies': ['companies.view', 'companies.manage'],
      'nav.branches': ['branches.view', 'branches.manage'],
      'nav.costCenters': ['costCenters.view', 'costCenters.manage'],
      'nav.people': ['members.view', 'suppliers.view', 'customers.view'],
      'nav.members': ['members.view', 'members.manage'],
      'nav.suppliers': ['suppliers.view', 'suppliers.manage'],
      'nav.supplierCategories': ['supplierCategories.view', 'supplierCategories.manage'],
      'nav.customers': ['customers.view', 'customers.manage'],
      'nav.customerCategories': ['customerCategories.view', 'customerCategories.manage'],
      'nav.userManagement': ['users.view', 'users.manage', 'roles.view', 'roles.manage'],
      'nav.users': ['users.view', 'users.manage'],
      'nav.roles': ['roles.view', 'roles.manage'],
      'nav.hrm': ['staff.view', 'departments.view', 'leave.view', 'attendance.view', 'payroll.view', 'recruitment.view', 'performance.view'],
      'nav.programs': ['plans.view', 'classes.view'],
      'nav.plans': ['plans.view', 'plans.manage'],
      'nav.classes': ['classes.view', 'classes.manage'],
      'nav.accounting': ['accounting.settings', 'accounting.reports', 'accounting.receiptVoucher', 'accounting.paymentVoucher', 'accounting.journalVoucher'],
      'nav.inventory': ['inventory.view', 'inventory.manage', 'pos.use'],
      'nav.assets': ['assets.register', 'assets.depreciation', 'assets.transactions', 'assets.setup', 'assets.reports'],
      'nav.sales': ['sales.list', 'sales.pos', 'sales.proposals', 'sales.estimates', 'sales.orders', 'sales.invoices', 'sales.shipments', 'sales.discounts', 'sales.returns', 'sales.reports'],
      'nav.pos2': ['sales.pos', 'sales.list'],
      'nav.purchases': ['purchases.orders', 'purchases.list', 'purchases.returns', 'purchases.reports', 'purchases.requisitions', 'purchases.goodsreceipts', 'purchases.supplierinvoices', 'purchases.supplierpayments'],
      'nav.reports': ['reports.view'],
      'nav.communications': ['notifications.manage'],
      'nav.notifications': ['notifications.manage'],
      'nav.checkin': ['checkin.use'],
      'nav.frontCms': ['settings.manage'],
      'nav.settings': ['settings.manage'],
      'nav.profile': ['dashboard.view'],
    }
    const customCanSee = (key?: string) => {
      if (!customDefinition || customDefinition.portal !== 'admin') return false
      const required = key ? CUSTOM_NAV_PERMISSIONS[key] : undefined
      return Boolean(required?.some((permission) => customDefinition.permissions.includes(permission)))
    }
    ...
    const canSee = (roles: Role[], key?: string) => {
      ...
      if (customCanSee(key)) return true
    ...
    return NAV.filter((i) => canSee(i.roles, i.key))
      ...
      .sort((a, b) => {
        const rankOf = (key: string) => {
          ...
          // Communications has no module entry (so no rank in the saved
          // sidebar order) — pin it directly after Project Management.
          if (key === 'nav.communications') return navRank(sidebarOrder, 'nav.projects') + 0.5
          // Front CMS has no module entry (so no rank in the saved sidebar
          // order) — pin it directly above the Settings menu.
          if (key === 'nav.frontCms') return navRank(sidebarOrder, 'nav.settings') - 0.5
          return navRank(sidebarOrder, key)
        }
        return rankOf(a.key) - rankOf(b.key)
      })
  }, [user, modules, roles, sidebarOrder])
  ...
  // The counter terminal owns the whole screen — no sidebar, header, or footer.
  if (loc.pathname === '/admin/pos' || loc.pathname === '/admin/pos2') {
    return (
          ...
          <div className={cn('px-3 pb-3', mini && 'hidden')}>
            <div className="sidebar-card rounded-xl border border-black/5 bg-zinc-50 px-3 py-2.5 dark:border-white/5 dark:bg-white/3">
              ...
              <div className="mt-0.5 flex items-center justify-between gap-2">
                <span className="truncate text-[11px] text-lime">{roleName(user.role, roles)}</span>
                <Link
                  to={`${roleHome(user.role)}/profile`}
                  title={t('nav.profile')}
                  className="shrink-0 rounded-md px-1 py-0.5 text-[11px] font-bold text-mist transition hover:bg-black/5 hover:text-lime dark:hover:bg-white/10"
                >
                  {t('nav.profile')}
                </Link>
              </div>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          >
            ...
            <div className="ml-auto flex items-center gap-1">
              ...
              {showOrgContext && canSwitchCompany && tenantCompanies.length > 1 && (
                <div className="relative hidden sm:block">
                  ...
                  >
                    ...
                    <span className="max-w-[10rem] truncate">{scopedCompany?.name || 'Company'}</span>
                      ...
                      {tenantCompanies.map((c) => (
                        <button
                          ...
                          className={cn(
                            ...
                            c.id === scopedCompany?.id && 'text-lime',
                          )}
                        >
              ...
              {isAssignedBranchUser && (
                <div className="flex max-w-[13rem] items-center gap-1.5 rounded-lg border border-line px-2.5 py-1.5 text-xs font-semibold" title="Assigned home branch">
                  <MapPin className="size-3.5 shrink-0 text-mist" />
                  <span className="truncate">{assignedBranch?.name || 'Branch not assigned'}</span>
                </div>
              )}
              {showOrgContext && !isAssignedBranchUser && (isBranchScoped || companyBranches.length > 1) && (
                <div className="relative hidden sm:block">
                  <button
                  ...
                  >
                    ...
                    <span className="max-w-[10rem] truncate">{isBranchScoped ? (selectedBranch?.name || 'Branch') : (selectedBranch?.name || 'All branches')}</span>
                  ...
                  {showBranch && !isBranchScoped && (
                    <div className="menu-pop absolute right-0 mt-2 w-64 rounded-2xl p-2">
                      ...
                      {branchesTreeOrder(companyBranches).map((b) => {
                        const depth = branchDepth(companyBranches, b.id)
                        return (
                          <button
                            key={b.id}
                          ...
                          >
                            <MapPin className={cn('text-mist', depth > 0 ? 'size-3' : 'size-3.5')} />
                            <span className="truncate">{depth > 0 ? '↳ ' : ''}{b.name}</span>
...
function useNavBadges(): Record<string, number> {
  ...
  return {
    overdueInvoices: invoices.filter((i) => i.status === 'overdue' || ((i.status === 'unpaid' || i.status === 'partially_paid') && i.dueAt < today)).length,
  }
}
