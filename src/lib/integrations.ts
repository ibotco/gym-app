...
export function catalog(): IntegrationRecord[] {
  return [
    ...
    seedOne({
      ...
      category: 'payments', version: '2024-11', critical: true, active: true, connected: true,
      health: 'online', apiStatus: 'Operational', lastSyncAt: iso(0, 8), lastSuccessAt: iso(0, 8),
      lastHealthCheckAt: iso(0, 8), lastTestMs: 96, lastTestResult: 'Authentication Successful',
      createdAt: iso(500, 9), updatedAt: iso(8, 10),
      config: { environment: 'production', webhookUrl: 'https://fitpro.gym/hooks/stripe', extra: { publishable: '' } },
    }),
    seedOne({
      id: 'int_paypal', name: 'PayPal', provider: 'PayPal',
      description: 'Wallet checkout for diaspora members paying in GHS.',
      category: 'payments', version: '2.0.4', critical: false, active: true, connected: true,
      health: 'online', apiStatus: 'Operational', lastSyncAt: iso(1, 15), lastSuccessAt: iso(1, 15),
      lastHealthCheckAt: iso(1, 15), lastTestMs: 240, lastTestResult: 'Authentication Successful',
      createdAt: iso(450, 10), updatedAt: iso(20, 9),
