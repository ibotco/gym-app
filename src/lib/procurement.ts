...
/** Only draft/rejected orders are editable; everything later is locked. */
...
export function fullInvoiceTotals(
  ...
  return {
    ...
    expenseTax,
    taxTotal,
    total: itemSubtotal + expenseSubtotal + taxTotal,
  }
}
...
/** Posted invoices are locked; everything earlier in the workflow is editable. */
...
export function invoiceJournalPreview(
  ...
  accounts: Account[],
  opts: { inventoryAccountId?: string; payableAccountId?: string; taxAccountId?: string } = {},
): JournalLinePreview[] {
  ...
  const t = fullInvoiceTotals(items, expenses)
  const out: JournalLinePreview[] = []

  if (t.itemSubtotal > 0) {
    out.push({ accountId: inventoryId, accountName: nameOf(inventoryId), debit: t.itemSubtotal, credit: 0 })
  }
  ...
  for (const e of expenses) {
    ...
    byAccount.set(e.accountId, (byAccount.get(e.accountId) || 0) + amt)
  }
  ...
  if (t.total > 0) {
    out.push({ accountId: payableId, accountName: nameOf(payableId), debit: 0, credit: t.total })
  }
  return out
}
