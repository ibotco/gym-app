...
import type {
  ...
  ChequeEntry,
  JournalVoucher, JournalTemplate, PaymentVoucher, ReceiptVoucher, SupplierPayment, SupplierInvoice, GoodsReceipt, ValueBookEntry, VoucherSerial, Fund, PaymentModeOption, AccountDetailType, IncomeStatementMod, CurrencyRate, AccountType, ReceiptLine,
  MappingVoucherType, MappingJournal, MappingPostingProfile, AutoMappingLine,
} from '../types'
...
export const JOURNAL_TEMPLATES_KEY = 'fitpro_journal_templates'
...
/**
 * Default ledger accounts used when a transaction is posted automatically.
 *
 * Posting convention (matches the Mapping Setup UI columns):
 *   paymentAccountId   = LEFT  column "Payment account" = the CREDIT side
 *   depositToAccountId = RIGHT column "Deposit to"      = the DEBIT side
 *
 * Each entry below is annotated as `Cr <account> / Dr <account>` so the
 * double entry can be checked at a glance.
 */
export const DEFAULT_AUTO_MAPPING_LINES: Record<string, {
  ...
  // ── General ──────────────────────────────────────────────────────────────
  // Raise an invoice: earn revenue, create a receivable.
  'invoice.default': {
  ...
  // Discount given on an invoice.
  'invoice.discount': {
    paymentAccountId: 'ac_1', // Accounts Receivable (A/R)
    depositToAccountId: 'ac_19', // Discounts given - COS
    enabled: true,
  },
  // Customer settles an invoice.
  'payment.sales': {
    paymentAccountId: 'ac_1', // Accounts Receivable (A/R)
    depositToAccountId: 'ac_13', // Cash and cash equivalents
    enabled: true,
  },
  // Paying an expense: cash leaves, the expense is recognised.
  'payment.expenses': {
    paymentAccountId: 'ac_13', // Cr Cash and cash equivalents
    depositToAccountId: 'ac_80', // Dr Uncategorised Expense
    enabled: true,
  },
  // Credit note against a sale: reverse the revenue, reduce the receivable.
  'creditnote.sales': {
    paymentAccountId: 'ac_1', // Cr Accounts Receivable (A/R)
    depositToAccountId: 'ac_66', // Dr Sales
    enabled: true,
  },
  // Refund paid to the customer: cash out, clear the credit balance.
  'creditnote.refund': {
    paymentAccountId: 'ac_13', // Cr Cash and cash equivalents
    depositToAccountId: 'ac_1', // Dr Accounts Receivable (A/R)
    enabled: true,
  },
  // Standalone expense.
  'expense.default': {
  ...
  // ── Payslips ─────────────────────────────────────────────────────────────
  // Insurance withheld/accrued on a payslip: owed on, cost recognised.
  'payslip.insurance': {
    paymentAccountId: 'ac_57', // Cr Payroll liabilities
    depositToAccountId: 'ac_31', // Dr Insurance - General
    enabled: true,
  },
  // PAYE withheld from staff: a payroll liability, not company income tax.
  'payslip.tax': {
    paymentAccountId: 'ac_57', // Cr Payroll liabilities
    depositToAccountId: 'ac_85', // Dr Wage expenses
    enabled: true,
  },
  // Net pay disbursed to the employee.
  'payslip.netpay': {
    paymentAccountId: 'ac_13', // Cr Cash and cash equivalents
    depositToAccountId: 'ac_56', // Dr Payroll Expenses
    enabled: true,
  },

  // ── Purchase ─────────────────────────────────────────────────────────────
  // Goods placed on order against a supplier account.
  'purchase.order': {
    paymentAccountId: 'ac_87', // Cr Accounts Payable (A/P)
    depositToAccountId: 'ac_36', // Dr Inventory
    enabled: true,
  },
  // Goods physically received: stock in, obligation accrued in GRNI until the
  // supplier's bill arrives.
  'purchase.goodsreceipt': {
    paymentAccountId: 'ac_101', // Cr Goods Received Not Invoiced (GRNI)
    depositToAccountId: 'ac_36', // Dr Inventory
    enabled: true,
  },
  // Supplier invoice received: creates a payable, not a cash movement.
  'purchase.invoice': {
    paymentAccountId: 'ac_87', // Cr Accounts Payable (A/P)
    depositToAccountId: 'ac_60', // Dr Purchases
    enabled: true,
  },
  // Paying the supplier: cash out, payable cleared.
  'purchase.payment': {
    paymentAccountId: 'ac_13', // Cr Cash and cash equivalents
    depositToAccountId: 'ac_87', // Dr Accounts Payable (A/P)
    enabled: true,
  },
  // Returning goods to the supplier: stock out, payable reduced.
  'purchase.return': {
    paymentAccountId: 'ac_36', // Cr Inventory
    depositToAccountId: 'ac_87', // Dr Accounts Payable (A/P)
    enabled: true,
  },
  // Supplier refunds us: payable cleared, cash in.
  'purchase.refund': {
    paymentAccountId: 'ac_87', // Cr Accounts Payable (A/P)
    depositToAccountId: 'ac_13', // Dr Cash and cash equivalents
    enabled: true,
  },
  // Input tax on a purchase invoice: accrues with the payable.
  'purchase.tax': {
    paymentAccountId: 'ac_87', // Cr Accounts Payable (A/P)
    depositToAccountId: 'ac_29', // Dr Income tax payable
    enabled: true,
  },

  // ── Inventory ────────────────────────────────────────────────────────────
  // Goods received note: stock in, supplier owed.
  'inventory.receiving': {
    paymentAccountId: 'ac_87', // Cr Accounts Payable (A/P)
    depositToAccountId: 'ac_36', // Dr Inventory
    enabled: true,
  },
  // Customer return back into stock: reverses the delivery entry.
  'inventory.return': {
    paymentAccountId: 'ac_16', // Cr Cost of sales
    depositToAccountId: 'ac_36', // Dr Inventory
    enabled: true,
  },
  // Delivery to customer: stock relieved, cost of sale recognised.
  'inventory.delivery': {
    paymentAccountId: 'ac_36', // Cr Inventory
    depositToAccountId: 'ac_16', // Dr Cost of sales
    enabled: true,
  },
  // Margin recognised on the delivery.
  'inventory.profit': {
    paymentAccountId: 'ac_66', // Cr Sales
    depositToAccountId: 'ac_1', // Dr Accounts Receivable (A/R)
    enabled: true,
  },
  // Stock adjustment upward (gain).
  'inventory.increase': {
    paymentAccountId: 'ac_14', // Cr Change in inventory - COS
    depositToAccountId: 'ac_36', // Dr Inventory
    enabled: true,
  },
  // Stock adjustment downward (shrinkage/loss).
  'inventory.decrease': {
    paymentAccountId: 'ac_36', // Cr Inventory
    depositToAccountId: 'ac_14', // Dr Change in inventory - COS
    enabled: true,
  },
  // Opening stock brought on to the books at go-live.
  'inventory.openingstock': {
    paymentAccountId: 'ac_89', // Cr Opening Balance Equity
    depositToAccountId: 'ac_36', // Dr Inventory
    enabled: true,
  },

  // ── Manufacturing ────────────────────────────────────────────────────────
  // Raw materials issued to a production order.
  'mfg.material': {
    paymentAccountId: 'ac_36', // Cr Inventory
    depositToAccountId: 'ac_45', // Dr Materials - COS
    enabled: true,
  },
  // Direct labour absorbed into the production order.
  'mfg.labour': {
    paymentAccountId: 'ac_55', // Cr Payroll Clearing
    depositToAccountId: 'ac_18', // Dr Direct labour - COS
    enabled: true,
  },
  // ── Fixed equipment ──────────────────────────────────────────────────────
  // Buying equipment: cash out, capitalised to property, plant and equipment.
  'fixedequip.asset': {
    paymentAccountId: 'ac_13', // Cr Cash and cash equivalents
    depositToAccountId: 'ac_59', // Dr Property, plant and equipment
    enabled: true,
  },
  // Software or operating licences: an intangible asset.
  'fixedequip.license': {
    paymentAccountId: 'ac_13', // Cr Cash and cash equivalents
    depositToAccountId: 'ac_33', // Dr Intangibles
    enabled: true,
  },
  // Capitalised component or spare part fitted to an asset.
  'fixedequip.component': {
    paymentAccountId: 'ac_13', // Cr Cash and cash equivalents
    depositToAccountId: 'ac_59', // Dr Property, plant and equipment
    enabled: true,
  },
  // Consumables are used up, so they are expensed rather than capitalised.
  'fixedequip.consumables': {
    paymentAccountId: 'ac_13', // Cr Cash and cash equivalents
    depositToAccountId: 'ac_75', // Dr Supplies
    enabled: true,
  },
  // Servicing keeps an asset running; it does not add value, so it is expensed.
  'fixedequip.maintenance': {
    paymentAccountId: 'ac_13', // Cr Cash and cash equivalents
    depositToAccountId: 'ac_63', // Dr Repair and maintenance
    enabled: true,
  },
  // Depreciation moves no cash: build up the contra-asset, charge the expense.
  'fixedequip.depreciation': {
    paymentAccountId: 'ac_5', // Cr Accumulated depreciation on PP&E
    depositToAccountId: 'ac_100', // Dr Depreciation expense
    enabled: true,
  },

  // ── Procurement (GRN-based purchasing) ─────────────────────────────────
  // Goods receipt: goods are on hand but not yet invoiced, so the credit sits
  // in GRNI rather than Accounts Payable.
  'procurement.goodsreceipt': {
    paymentAccountId: 'ac_101', // Cr Goods Received Not Invoiced (GRNI)
    depositToAccountId: 'ac_36', // Dr Inventory
    enabled: true,
  },
  // Supplier invoice clears GRNI and raises the real payable.
  'procurement.supplierinvoice': {
    paymentAccountId: 'ac_87', // Cr Accounts Payable (A/P)
    depositToAccountId: 'ac_101', // Dr Goods Received Not Invoiced (GRNI)
    enabled: true,
  },
  // Paying the supplier settles the payable out of cash.
  'procurement.supplierpayment': {
    paymentAccountId: 'ac_13', // Cr Cash and cash equivalents
    depositToAccountId: 'ac_87', // Dr Accounts Payable (A/P)
    enabled: true,
  },
  // Returning goods reverses the inventory and reduces what is owed.
...
// ---------------------------------------------------------------------------
// Voucher / journal / posting-profile links for each mapping record.
//
// Every mapping key gets a reviewed default: which voucher document the event
// is recorded as, which journal the ledger entry belongs to, and how the
// generated voucher is handled (auto-posted or kept as a draft). Users can
// override any of the three per record in Settings → Mapping Setup.
// ---------------------------------------------------------------------------

export const MAPPING_VOUCHER_TYPES: { value: MappingVoucherType; label: string }[] = [
  { value: 'receipt', label: 'Receipt voucher' },
  { value: 'payment', label: 'Payment voucher' },
  { value: 'journal', label: 'Journal entry' },
]

export const MAPPING_JOURNALS: { value: MappingJournal; label: string }[] = [
...
export const MAPPING_POSTING_PROFILES: { value: MappingPostingProfile; label: string; hint: string }[] = [
  { value: 'auto_post', label: 'Auto post', hint: 'The voucher is posted to the ledger as soon as the event occurs.' },
  { value: 'manual_draft', label: 'Keep as draft', hint: 'The voucher is saved as a draft for review before posting.' },
]

const LINK = (voucherType: MappingVoucherType, journal: MappingJournal, postingProfile: MappingPostingProfile) => ({
  voucherType, journal, postingProfile,
})

export const DEFAULT_MAPPING_LINKS: Record<string, {
  voucherType: MappingVoucherType
  journal: MappingJournal
  postingProfile: MappingPostingProfile
}> = {
  // ── General ──────────────────────────────────────────────────────────────
  'invoice.default': LINK('receipt', 'sales', 'auto_post'),
  'invoice.discount': LINK('receipt', 'sales', 'auto_post'),
  'payment.sales': LINK('receipt', 'sales', 'auto_post'),
  'payment.expenses': LINK('payment', 'cash', 'auto_post'),
  'creditnote.sales': LINK('journal', 'sales', 'manual_draft'),
  'creditnote.refund': LINK('payment', 'cash', 'manual_draft'),
  'expense.default': LINK('payment', 'cash', 'auto_post'),
  'tax.sales': LINK('journal', 'sales', 'auto_post'),
  'tax.expenses': LINK('journal', 'purchases', 'auto_post'),
  // ── Payslips ─────────────────────────────────────────────────────────────
  'payslip.insurance': LINK('payment', 'general', 'auto_post'),
  'payslip.tax': LINK('payment', 'general', 'auto_post'),
  'payslip.netpay': LINK('payment', 'general', 'auto_post'),
  // ── Purchase ─────────────────────────────────────────────────────────────
  'purchase.order': LINK('journal', 'purchases', 'manual_draft'),
  'purchase.goodsreceipt': LINK('journal', 'purchases', 'auto_post'),
  'purchase.invoice': LINK('journal', 'purchases', 'auto_post'),
  'purchase.payment': LINK('payment', 'purchases', 'auto_post'),
  'purchase.return': LINK('journal', 'purchases', 'manual_draft'),
  'purchase.refund': LINK('receipt', 'purchases', 'manual_draft'),
  'purchase.tax': LINK('journal', 'purchases', 'auto_post'),
  // ── Inventory ────────────────────────────────────────────────────────────
  'inventory.receiving': LINK('journal', 'purchases', 'auto_post'),
  'inventory.return': LINK('journal', 'sales', 'manual_draft'),
  'inventory.delivery': LINK('journal', 'sales', 'auto_post'),
  'inventory.profit': LINK('journal', 'sales', 'auto_post'),
  'inventory.increase': LINK('journal', 'general', 'manual_draft'),
  'inventory.decrease': LINK('journal', 'general', 'manual_draft'),
  'inventory.openingstock': LINK('journal', 'general', 'manual_draft'),
  // ── Manufacturing ────────────────────────────────────────────────────────
  'mfg.material': LINK('journal', 'general', 'manual_draft'),
  'mfg.labour': LINK('journal', 'general', 'manual_draft'),
  // ── Fixed equipment ──────────────────────────────────────────────────────
  'fixedequip.asset': LINK('journal', 'general', 'manual_draft'),
  'fixedequip.license': LINK('journal', 'general', 'manual_draft'),
  'fixedequip.component': LINK('journal', 'general', 'manual_draft'),
  'fixedequip.consumables': LINK('journal', 'general', 'manual_draft'),
  'fixedequip.maintenance': LINK('payment', 'general', 'auto_post'),
  'fixedequip.depreciation': LINK('journal', 'general', 'manual_draft'),
  // ── Procurement ──────────────────────────────────────────────────────────
  'procurement.goodsreceipt': LINK('journal', 'purchases', 'auto_post'),
  'procurement.supplierinvoice': LINK('journal', 'purchases', 'auto_post'),
  'procurement.supplierpayment': LINK('payment', 'purchases', 'auto_post'),
  'procurement.purchasereturn': LINK('journal', 'purchases', 'manual_draft'),
}

/** Display label for a stored mapping link value. */
export function mappingLinkLabel(kind: 'voucherType' | 'journal' | 'postingProfile', value: string | undefined): string {
  const table = kind === 'voucherType' ? MAPPING_VOUCHER_TYPES : kind === 'journal' ? MAPPING_JOURNALS : MAPPING_POSTING_PROFILES
  return (table as { value: string; label: string }[]).find((x) => x.value === value)?.label || '—'
}

/**
 * Resolved voucher / journal / posting-profile link for a mapping key:
 * saved value → reviewed default → safe fallback.
 */
export function mappingLink(
  settings: AccountingSettings,
  key: string,
): { voucherType: MappingVoucherType; journal: MappingJournal; postingProfile: MappingPostingProfile } {
  const line = settings.autoMapping?.lines?.[key]
  const def = DEFAULT_MAPPING_LINKS[key]
  return {
    voucherType: line?.voucherType || def?.voucherType || 'journal',
    journal: line?.journal || def?.journal || 'general',
    postingProfile: line?.postingProfile || def?.postingProfile || 'auto_post',
  }
}
...
export function loadAccountingSettings(): AccountingSettings {
  ...
  const mergedLines: Record<string, AutoMappingLine> = { ...DEFAULT_AUTO_MAPPING_LINES }
  if (autoMapping?.lines) {
    const savedLines = autoMapping.lines as Record<string, Partial<AutoMappingLine>>
    for (const [key, def] of Object.entries(DEFAULT_AUTO_MAPPING_LINES)) {
      const savedLine = savedLines[key]
      const link = DEFAULT_MAPPING_LINKS[key]
      // Reviewed voucher/journal/profile defaults sit under any saved value,
      // so older saves pick them up automatically and edits still win.
      mergedLines[key] = savedLine
        ? { ...def, ...link, ...savedLine, hint: def.hint }
        : { ...def, ...link }
    }
  } else {
    for (const [key, def] of Object.entries(DEFAULT_AUTO_MAPPING_LINES)) {
      mergedLines[key] = { ...def, ...DEFAULT_MAPPING_LINKS[key] }
    }
  }
  return {
    ...base,
...
/**
 * Ledger rows for a posted Goods Receipt (GRN):
 *   Dr Inventory / Cr Goods Received Not Invoiced (GRNI)
 *
 * Receiving goods creates an obligation before the supplier's bill arrives, so
 * the credit accrues in GRNI rather than Accounts Payable; the purchase invoice
 * later debits GRNI to clear it. Value is the ACCEPTED quantity only — rejected
 * units never entered stock, so they must not be valued here either. Only
 * posted receipts reach the ledger; drafts and cancellations clear their rows.
 */
export function historyRowsForGoodsReceipt(
  grn: GoodsReceipt,
  opts?: { supplierName?: string; settings?: AccountingSettings },
): AccountHistoryEntry[] {
  if (grn.status !== 'posted') return []
  const settings = opts?.settings ?? loadAccountingSettings()
  // Two tabs can configure this entry: the Purchase tab's "Good receipts" row
  // and the Procurement tab's equivalent. Prefer the Purchase row when it has
  // been configured, falling back to Procurement, then to the built-in pair.
  const lines = settings.autoMapping?.lines || {}
  const line = lines['purchase.goodsreceipt'] || lines['procurement.goodsreceipt']
  const grni = line?.paymentAccountId || 'ac_101' // Cr GRNI
  const inventoryAccount = line?.depositToAccountId || 'ac_36' // Dr Inventory
  // Voucher/journal link from the mapping record that won.
  const link = mappingLink(settings, lines['purchase.goodsreceipt'] ? 'purchase.goodsreceipt' : 'procurement.goodsreceipt')

  // Accepted quantity only — mirrors the stock actually moved on posting.
  const value = (grn.lines || []).reduce(
    (sum, l) => sum + (Number(l.quantityReceiving) || 0) * (Number(l.unitCost) || 0),
    0,
  )
  if (!(value > 0)) return []
  ...
  const now = new Date().toISOString()
  const shared = {
    description: `Goods receipt ${grn.number}`,
    relId: grn.id,
    relType: 'goods_receipt',
    dateCreated: grn.createdAt || now,
    addedFrom: grn.postedBy,
    vendor: opts?.supplierName || undefined,
    date: grn.date,
    number: grn.number,
  ...
  return [
    { ...shared, id: `ah_grn_${grn.id}_dr`, account: inventoryAccount, debit: value, credit: 0, split: grni },
    { ...shared, id: `ah_grn_${grn.id}_cr`, account: grni, debit: 0, credit: value, split: inventoryAccount },
  ]
}

/** Statuses at which a supplier invoice has hit the ledger. */
const POSTED_INVOICE_STATUSES = new Set(['posted', 'partially_paid', 'paid'])

/**
 * Ledger rows for a posted Supplier Invoice (procurement module).
 *
 *   Cr Accounts Payable            — the full invoice total becomes owed
 *   Dr GRNI  (goods already received via a GRN)
 *      or Purchases (invoice raised without a goods receipt)
 *   Dr each expense line's own account
 *   Dr input tax                   — when the invoice carries tax
 *
 * Accounts come from the configurable mapping. Item lines follow
 * `procurement.supplierinvoice` when the invoice cites a GRN, clearing the
 * GRNI accrual the receipt raised; otherwise they fall back to
 * `purchase.invoice` so the cost still lands somewhere real. Draft, submitted,
 * approved and cancelled invoices never reach the ledger — only posting does.
 */
export function historyRowsForSupplierInvoice(
  inv: SupplierInvoice,
  opts?: { supplierName?: string; settings?: AccountingSettings },
): AccountHistoryEntry[] {
  if (!POSTED_INVOICE_STATUSES.has(inv.status)) return []
  const settings = opts?.settings ?? loadAccountingSettings()
  const lines = settings.autoMapping?.lines || {}
  const procLine = lines['procurement.supplierinvoice']
  const purchLine = lines['purchase.invoice']
  const taxLine = lines['purchase.tax']

  const payable = procLine?.paymentAccountId || 'ac_87'
  // Goods already received sit in GRNI; a standalone invoice expenses instead.
  const itemAccount = inv.goodsReceiptId
    ? (procLine?.depositToAccountId || 'ac_101')
    : (purchLine?.depositToAccountId || 'ac_60')
  const taxAccount = taxLine?.depositToAccountId || 'ac_29'

  const now = new Date().toISOString()
  // Voucher/journal link from the mapping record that won for the item lines.
  const link = mappingLink(settings, inv.goodsReceiptId ? 'procurement.supplierinvoice' : 'purchase.invoice')
  const shared = {
    description: `Supplier invoice ${inv.number}`,
    relId: inv.id,
    relType: 'supplier_invoice',
    dateCreated: inv.createdAt || now,
    addedFrom: inv.postedBy,
    vendor: opts?.supplierName || undefined,
  ...
  const rows: AccountHistoryEntry[] = []
  // Net of item lines after discount, excluding tax.
  const itemNet = (inv.lines || []).reduce((sum, l) => {
    const gross = (Number(l.quantity) || 0) * (Number(l.unitCost) || 0)
    return sum + Math.max(0, gross - gross * ((Number(l.discountPercent) || 0) / 100))
  }, 0)
  const expenseNet = (inv.expenseLines || []).reduce((sum, e) => sum + (Number(e.amount) || 0), 0)
  // An order-level discount is shared pro-rata across the item/expense debits so
  // the payable credit stays exactly the invoice total.
  const discountBase = itemNet + expenseNet
  const orderDisc = Math.min(Math.max(0, Number(inv.orderDiscount) || 0), discountBase)
  const factor = discountBase > 0 ? (discountBase - orderDisc) / discountBase : 0
  if (itemNet * factor > 0) {
    rows.push({ ...shared, id: `ah_sinv_${inv.id}_items`, account: itemAccount, debit: itemNet * factor, credit: 0, split: payable })
  }
  // Each expense line debits the account chosen on the line itself.
  ;(inv.expenseLines || []).forEach((e, i) => {
    const amount = Number(e.amount) || 0
    if (!e.accountId || amount <= 0) return
    rows.push({
      ...shared,
      id: `ah_sinv_${inv.id}_exp${i}`,
      account: e.accountId,
      debit: amount * factor,
      credit: 0,
      split: payable,
      description: e.description || shared.description,
    })
  })
  const tax = Number(inv.taxTotal) || 0
  if (tax > 0) {
    rows.push({ ...shared, id: `ah_sinv_${inv.id}_tax`, account: taxAccount, debit: tax, credit: 0, split: payable, tax })
  }
  const orderTax = Number(inv.orderTax) || 0
  if (orderTax > 0) {
    rows.push({ ...shared, id: `ah_sinv_${inv.id}_ortax`, account: taxAccount, debit: orderTax, credit: 0, split: payable, tax: orderTax })
  }
  if (!rows.length) return []
  // Credit the payable with exactly the sum of the debits so the entry always
  // balances, even if a stored total drifted from its lines.
  const debitTotal = rows.reduce((sum, r) => sum + r.debit, 0)
  rows.push({ ...shared, id: `ah_sinv_${inv.id}_ap`, account: payable, debit: 0, credit: debitTotal, split: rows[0].account })
  return rows
}

/**
 * Ledger rows for a posted Supplier Payment (procurement module):
 *   Dr Accounts Payable / Cr the bank or cash account that funded it.
 *
 * The accounts come from the configurable mapping (Accounting → Settings →
 * Mapping → Purchase → Supplier Payment), except that the credit side prefers
 * the account actually chosen on the payment itself. Only POSTED payments
 * reach the ledger — drafts and cancelled payments never appear.
 */
export function historyRowsForSupplierPayment(
  sp: SupplierPayment,
  opts?: { supplierName?: string; settings?: AccountingSettings },
): AccountHistoryEntry[] {
  if (sp.status !== 'posted') return []
  if (!(sp.amount > 0)) return []
  const settings = opts?.settings ?? loadAccountingSettings()
  const line = settings.autoMapping?.lines?.['purchase.payment']
  // Cr: money leaving — the account picked on the payment wins over the map.
  const creditAccount = sp.accountId || line?.paymentAccountId || 'ac_13'
  // Dr: the payable being cleared.
  const debitAccount = line?.depositToAccountId || 'ac_87'
  const now = new Date().toISOString()
  const vendor = opts?.supplierName || undefined
  // Voucher/journal link from the supplier payment mapping record.
  const link = mappingLink(settings, 'purchase.payment')
  const shared = {
    description: sp.reference ? `Supplier payment · ${sp.reference}` : 'Supplier payment',
    relId: sp.id,
    relType: 'supplier_payment',
    dateCreated: sp.createdAt || now,
    addedFrom: sp.postedBy,
    vendor,
    date: sp.paymentDate,
    number: sp.number,
    voucherType: link.voucherType,
  ...
  return [
    { ...shared, id: `ah_spay_${sp.id}_dr`, companyId: sp.companyId, branchId: sp.branchId, account: debitAccount, debit: sp.amount, credit: 0, split: creditAccount },
    { ...shared, id: `ah_spay_${sp.id}_cr`, companyId: sp.companyId, branchId: sp.branchId, account: creditAccount, debit: 0, credit: sp.amount, split: debitAccount },
  ]
}
...
export function buildAccountHistory(
  ...
  return [
    ...
    ...supplierPayments.flatMap((sp) => historyRowsForSupplierPayment(sp)),
    ...supplierInvoices.flatMap((si) => historyRowsForSupplierInvoice(si)),
    ...goodsReceipts.flatMap((g) => historyRowsForGoodsReceipt(g)),
  ]
}
...
/** Reusable journal entry skeletons — selectable in the Journal Entry form. */
export const JOURNAL_TEMPLATES: JournalTemplate[] = [
  {
    id: 'jt_1', name: 'Rent payment', description: 'Monthly rent payment',
    lines: [
      { accountId: 'ac_62', debit: 3500, credit: 0 },
      { accountId: 'ac_88', debit: 0, credit: 3500 },
    ],
    createdAt: '2026-01-02T09:00:00',
  },
  {
    id: 'jt_2', name: 'Utilities settlement', description: 'Electricity & water bills',
    lines: [
      { accountId: 'ac_84', debit: 950, credit: 0 },
      { accountId: 'ac_13', debit: 0, credit: 950 },
    ],
    createdAt: '2026-01-02T09:05:00',
  },
  {
    id: 'jt_3', name: 'Depreciation — monthly', description: 'Monthly straight-line depreciation',
    lines: [
      { accountId: 'ac_100', debit: 620, credit: 0 },
      { accountId: 'ac_5', debit: 0, credit: 620 },
    ],
    createdAt: '2026-01-02T09:10:00',
  },
]

export const loadJournalTemplates = () => load(JOURNAL_TEMPLATES_KEY, JOURNAL_TEMPLATES)
export function saveJournalTemplates(rows: JournalTemplate[]) {
  save(JOURNAL_TEMPLATES_KEY, rows)
}
