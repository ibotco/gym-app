...
export function historyRowsForReceipt(r: ReceiptVoucher): AccountHistoryEntry[] {
  ...
  const rows: AccountHistoryEntry[] = [{
    id: `ah_income_${r.id}_0`, account: r.depositAccountId, debit: r.amount, credit: 0,
  ...
  ;(r.lines || []).forEach((l, i) => {
    ...
    rows.push({
      id: `ah_income_${r.id}_${i + 1}`, account: l.accountId, debit: 0, credit: l.amount,
...
export function historyRowsForPayment(pv: PaymentVoucher): AccountHistoryEntry[] {
  ...
  const now = new Date().toISOString()
  const rows: AccountHistoryEntry[] = [{
    id: `ah_expense_${pv.id}_0`, account: pv.paymentAccountId, debit: 0, credit: pv.amount,
  ...
  ;(pv.lines || []).forEach((l, i) => {
    if (!l.accountId) return
    rows.push({
      id: `ah_expense_${pv.id}_${i + 1}`, account: l.accountId, debit: l.amount, credit: 0,
...
export function historyRowsForJournal(j: JournalVoucher): AccountHistoryEntry[] {
  ...
  return j.lines
    ...
    .map((l, i) => {
      ...
      return {
        id: `ah_journal_${j.id}_${i}`, account: l.accountId, debit: l.debit || 0, credit: l.credit || 0,
