...
export function loadMatrix(): MatrixState {
  try {
    const raw = localStorage.getItem(MATRIX_KEY)
    if (raw) return JSON.parse(raw) as MatrixState
  } catch { /* ignore */ }
  return {}
}

export function saveMatrix(state: MatrixState) {
  try { localStorage.setItem(MATRIX_KEY, JSON.stringify(state)) } catch { /* ignore */ }
}
...
/** True if a role can manage the matrix (super admin or has roles.manage). */
export function canEditMatrix(role: RoleDef | null | undefined): boolean {
  return !!role && (role.id === 'super_admin' || role.permissions.includes('roles.manage'))
}
