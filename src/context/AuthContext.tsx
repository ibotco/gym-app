...
interface AuthCtx {
  ...
  logout: () => void
  register: (input: { name: string; email: string; password: string; phone: string; planId?: string; avatar?: string }) => Promise<{
    ok: boolean
    error?: string
    needsVerification?: boolean
    email?: string
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const setSession = (id: string | null) => {
    ...
    // AppContext sits above AuthProvider, so notify it when the active scope changes.
    window.dispatchEvent(new Event('fitpro-session-change'))
  }
  ...
  const value = useMemo<AuthCtx>(
    () => ({
      ...
      register: async ({ name, email, password, phone, planId, avatar }) => {
      ...
      hasPermission: (permission) => {
        ...
        const def = app.roles.find((r) => r.id === user.role)
        // app.permissions is already scoped to the signed-in user's company.
        // Requiring the key to resolve there prevents a stale or tampered role
        // from granting another tenant's custom permission by string alone.
        const permissionDef = app.permissions.find((candidate) => candidate.key === permission)
        return !!def && !!permissionDef && def.permissions.includes(permission)
      },
      impersonate: (id) => {
        if (user) {
...
export function roleHome(role: Role) {
  ...
  if (role === 'super_admin' || role === 'gym_manager' || role === 'staff' || role === 'accountant' || role === 'company_admin' || role === 'branch_admin' || role === 'receptionist' || role === 'head_office') return '/admin'
  // Custom role — look up its portal.
  const def = loadRoles().find((r) => r.id === role)
  if (def && !def.builtin) {
    if (def.portal === 'member') return '/app'
    if (def.portal === 'coach') return '/coach'
    if (def.portal === 'customer') return '/customer'
    if (def.portal === 'supplier') return '/supplier'
  }
  return '/admin'
}
