...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  }, [plans, log, setUsers, setInvoices, setPayments, paymentSettings])
  ...
  const upsertStakeholderEntity = useCallback((e: StakeholderEntity) => {
    setStakeholderEntitiesState((s) => { const n = s.some((x) => x.id === e.id) ? s.map((x) => (x.id === e.id ? e : x)) : [...s, e]; saveStakeholderEntities(n); return n })
  }, [])
  ...
  const upsertReceipt = useCallback((rv: ReceiptVoucher) => {
    upsertReceiptBase(rv)
    syncHistory('income', rv.id, historyRowsForReceipt(rv))
  }, [upsertReceiptBase, syncHistory])
  ...
  const upsertPaymentVoucher = useCallback((pv: PaymentVoucher) => {
    upsertPaymentVoucherBase(pv)
    syncHistory('expense', pv.id, historyRowsForPayment(pv))
  }, [upsertPaymentVoucherBase, syncHistory])
  ...
  const upsertJournal = useCallback((jv: JournalVoucher) => {
    upsertJournalBase(jv)
    syncHistory('journal_entry', jv.id, historyRowsForJournal(jv))
  }, [upsertJournalBase, syncHistory])
  ...
  const upsertCheque = makeUpsert<ChequeEntry>(setChequesState, saveCheques)
