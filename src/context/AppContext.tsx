...
import type {
  PurchaseRequisition, ProcPurchaseOrder, GoodsReceipt,
  SupplierInvoice, SupplierPayment, ProcPurchaseReturn,
  Attendance, AuditLog, Booking, Branch, GymClass, Invoice, InvoiceTemplate, Lead, LeaveRequest,
  ...
  Proposal, Estimate, SalesOrder, PurchaseOrder, PurchaseReturn, Shipment, Discount, SalesReturn, MessageTemplate,
  ...
  SystemSettings,
  AccountHistoryEntry,
  Account, AccountingSettings, ReceiptVoucher, PaymentVoucher, JournalVoucher, JournalTemplate,
...
import {
  ATTENDANCE, AUDIT, BOOKINGS, BRANCHES, CLASSES, COMPANY, INVOICES, INVOICE_TEMPLATES, LEADS, LEAVES,
...
import {
  loadRequisitions, saveRequisitions, loadProcPurchaseOrders, saveProcPurchaseOrders,
  loadGoodsReceipts, saveGoodsReceipts, nextPoStatus, receivedQty,
  loadSupplierInvoices, saveSupplierInvoices, loadSupplierPayments, saveSupplierPayments,
  loadProcReturns, saveProcReturns, returnedQty, invoicePayStatus, paymentInvoiceIds,
...
import { loadMessageTemplates, saveMessageTemplates, MESSAGE_TEMPLATES } from '../lib/messageTemplates'
...
import { loadAccountHistory, saveAccountHistory, historyRowsForReceipt, historyRowsForPayment, historyRowsForJournal, historyRowsForSupplierPayment, historyRowsForSupplierInvoice, historyRowsForGoodsReceipt } from '../lib/accounting'
...
import {
  ...
  loadReceipts, saveReceipts, loadPayments, savePayments, loadJournals, saveJournals, loadJournalTemplates, saveJournalTemplates,
  loadBanks, saveBanks, loadReconciliations, saveReconciliations, loadBudgets, saveBudgets,
  loadValueBook, saveValueBook, loadCheques, saveCheques, loadSignatories, saveSignatories, loadSerials, saveSerials, loadFunds, saveFunds, loadPaymentModes, savePaymentModes, loadDetailTypes, saveDetailTypes, loadIncomeMods, saveIncomeMods, loadCurrencyRates, saveCurrencyRates, ACCOUNTS, RECEIPTS, PAYMENTS as PAYMENT_VOUCHERS, JOURNALS, JOURNAL_TEMPLATES, BANKS, RECONCILIATIONS, BUDGETS, VALUE_BOOK, CHEQUES, SIGNATORIES, SERIALS, FUNDS, PAYMENT_MODES, DETAIL_TYPES, INCOME_MODS, CURRENCY_RATES, DEFAULT_ACCOUNTING_SETTINGS,
  ACCOUNTS_KEY, ACCT_SETTINGS_KEY, RECEIPTS_KEY, PAYMENTS_KEY, JOURNALS_KEY, JOURNAL_TEMPLATES_KEY, BANKS_KEY, RECON_KEY, BUDGETS_KEY, VALUEBOOK_KEY, CHECKS_KEY, SIGNATORIES_KEY, SERIALS_KEY, FUNDS_KEY, PAYMENT_MODES_KEY, DETAIL_TYPES_KEY, INCOME_MODS_KEY, CURRENCY_RATES_KEY,
} from '../lib/accounting'
import {
...
interface AppStore {
  ...
  procPurchaseOrders: ProcPurchaseOrder[]
  goodsReceipts: GoodsReceipt[]
  supplierInvoices: SupplierInvoice[]
  supplierPayments: SupplierPayment[]
  procReturns: ProcPurchaseReturn[]
  ...
  upsertInvoiceTemplate: (t: InvoiceTemplate) => void
  ...
  upsertRequisition: (r: PurchaseRequisition) => void
  deleteRequisition: (id: string) => void
  upsertProcPurchaseOrder: (o: ProcPurchaseOrder) => void
  deleteProcPurchaseOrder: (id: string) => void
  upsertGoodsReceipt: (g: GoodsReceipt) => void
  deleteGoodsReceipt: (id: string) => void
  /** Posts a GRN: validates against the PO, moves stock, advances the PO status. */
  /** `pending` lets a save-and-post flow pass the record directly, avoiding a stale-closure race. */
  postGoodsReceipt: (id: string, userName?: string, pending?: GoodsReceipt) => { ok: boolean; error?: string }
  upsertSupplierInvoice: (i: SupplierInvoice) => void
  deleteSupplierInvoice: (id: string) => void
  upsertSupplierPayment: (p: SupplierPayment) => void
  deleteSupplierPayment: (id: string) => void
  upsertProcReturn: (r: ProcPurchaseReturn) => void
  deleteProcReturn: (id: string) => void
  /** Posts a purchase return: validates against the GRN and reduces stock. */
  postProcReturn: (id: string, userName?: string, pending?: ProcPurchaseReturn) => { ok: boolean; error?: string }
  ...
  upsertMessageTemplate: (t: MessageTemplate) => void
  ...
  upsertJournalTemplate: (t: JournalTemplate) => void
  ...
  upsertCostCenter: (c: CostCenter) => void
...
const INVOICE_TEMPLATES_KEY = 'fitpro_invoice_templates'
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  // Invoice templates: plain load (not loadMerged) so deleting a seeded
  // template sticks instead of being re-merged on the next load.
  const [invoiceTemplates, setInvoiceTemplatesState] = useState<InvoiceTemplate[]>(() => {
    try {
  ...
  const setInvoiceTemplates = useCallback((action: InvoiceTemplate[] | ((prev: InvoiceTemplate[]) => InvoiceTemplate[])) => {
    setInvoiceTemplatesState((prev) => {
      const next = typeof action === 'function' ? action(prev) : action
      persistJson(INVOICE_TEMPLATES_KEY, next)
      return next
    })
  }, [])
  ...
  /** Latest suppliers, readable from callbacks without widening their deps. */
  ...
  const [requisitions, setRequisitionsState] = useState<PurchaseRequisition[]>(() => loadRequisitions())
  const [procPurchaseOrders, setProcPurchaseOrdersState] = useState<ProcPurchaseOrder[]>(() => loadProcPurchaseOrders())
  const [goodsReceipts, setGoodsReceiptsState] = useState<GoodsReceipt[]>(() => loadGoodsReceipts())
  const [supplierInvoices, setSupplierInvoicesState] = useState<SupplierInvoice[]>(() => loadSupplierInvoices())
  const [supplierPayments, setSupplierPaymentsState] = useState<SupplierPayment[]>(() => loadSupplierPayments())

  /**
   * Backfill: installs created before supplier payments posted to the ledger
   * have a saved history with no rows for them. Reconcile once on mount so
   * every already-posted payment shows up in Account History, and so any rows
   * left behind by a payment that was since deleted or unposted are removed.
   */
  useEffect(() => {
    setAccountHistoryState((current) => {
      const supplierName = (id: string) => suppliersRef.current.find((x) => x.id === id)?.name
      const wanted = [
        ...supplierPayments.flatMap((sp) => historyRowsForSupplierPayment(sp, { supplierName: supplierName(sp.supplierId) })),
        ...supplierInvoices.flatMap((si) => historyRowsForSupplierInvoice(si, { supplierName: supplierName(si.supplierId) })),
        ...goodsReceipts.flatMap((g) => historyRowsForGoodsReceipt(g, { supplierName: supplierName(g.supplierId) })),
      ]
      const OWNED_TYPES = new Set(['supplier_payment', 'supplier_invoice', 'goods_receipt'])
      const owned = (h: AccountHistoryEntry) => OWNED_TYPES.has(h.relType)
      const existing = current.filter(owned)
      const same = existing.length === wanted.length
        && wanted.every((w) => existing.some((e) => e.id === w.id && e.account === w.account && e.debit === w.debit && e.credit === w.credit && e.date === w.date))
      if (same) return current
      const next = [...current.filter((h) => !owned(h)), ...wanted]
      saveAccountHistory(next)
      return next
    })
    // Runs whenever either document set changes; cheap and idempotent.
  }, [supplierPayments, supplierInvoices, goodsReceipts])
  const [procReturns, setProcReturnsState] = useState<ProcPurchaseReturn[]>(() => loadProcReturns())
  ...
  const [messageTemplates, setMessageTemplatesState] = useState<MessageTemplate[]>(() => loadMessageTemplates())
  ...
  /** Replace every ledger row belonging to one source document. */
  ...
  const [journalTemplates, setJournalTemplatesState] = useState<JournalTemplate[]>(() => loadJournalTemplates())
  ...
  const adjustStock = useCallback((itemId: string, type: StockMovementType, quantity: number, reason: string) => {
    ...
    setInventoryState((s) => {
      ...
      if (item.nonInventory) { error = 'Non-inventory items are not stock-tracked. Change the product status first.'; return s }
  ...
  const upsertRequisition = useCallback((r: PurchaseRequisition) => {
    const scoped = stampSelectedOrg(r)
    setRequisitionsState((s) => {
      const n = s.some((x) => x.id === scoped.id) ? s.map((x) => (x.id === scoped.id ? scoped : x)) : [...s, scoped]
      saveRequisitions(n); return n
    })
  }, [stampSelectedOrg])

  const deleteRequisition = useCallback((id: string) => {
    setRequisitionsState((s) => { const n = s.filter((x) => x.id !== id); saveRequisitions(n); return n })
  }, [])

  const upsertProcPurchaseOrder = useCallback((o: ProcPurchaseOrder) => {
    const scoped = stampSelectedOrg(o)
    setProcPurchaseOrdersState((s) => {
      const n = s.some((x) => x.id === scoped.id) ? s.map((x) => (x.id === scoped.id ? scoped : x)) : [...s, scoped]
      saveProcPurchaseOrders(n); return n
    })
  }, [stampSelectedOrg])

  const deleteProcPurchaseOrder = useCallback((id: string) => {
    setProcPurchaseOrdersState((s) => { const n = s.filter((x) => x.id !== id); saveProcPurchaseOrders(n); return n })
  }, [])

  const upsertGoodsReceipt = useCallback((g: GoodsReceipt) => {
    const scoped = stampSelectedOrg(g)
    // Keep the ledger in step with the receipt's status (a draft or cancelled
    // receipt clears its rows; posting is handled in postGoodsReceipt).
    syncHistory('goods_receipt', scoped.id, historyRowsForGoodsReceipt(scoped, {
      supplierName: suppliersRef.current.find((x) => x.id === scoped.supplierId)?.name,
    }))
    setGoodsReceiptsState((s) => {
      const n = s.some((x) => x.id === scoped.id) ? s.map((x) => (x.id === scoped.id ? scoped : x)) : [...s, scoped]
      saveGoodsReceipts(n); return n
    })
  }, [stampSelectedOrg, syncHistory])

  const deleteGoodsReceipt = useCallback((id: string) => {
    syncHistory('goods_receipt', id, [])
    setGoodsReceiptsState((s) => { const n = s.filter((x) => x.id !== id); saveGoodsReceipts(n); return n })
  }, [syncHistory])

  /**
   * Post a Goods Receipt. This is the ONLY path in the procurement module that
   * moves stock — purchase orders never touch inventory.
   *
   * Validates the receipt against its purchase order (over-receipt guard),
   * increases inventory for each accepted line, writes a stock movement for the
   * audit trail, then advances the PO to partially/fully received.
   */
  const postGoodsReceipt = useCallback((id: string, userName?: string, pending?: GoodsReceipt) => {
    // Prefer the caller's record: when saving and posting in one action the
    // context state has not flushed yet, so a lookup by id would miss it.
    const grn = (pending && pending.id === id) ? pending : goodsReceipts.find((g) => g.id === id)
    if (!grn) return { ok: false, error: 'Goods receipt not found.' }
    if (grn.status === 'posted') return { ok: false, error: 'This receipt has already been posted.' }
    if (grn.status === 'cancelled') return { ok: false, error: 'This receipt has been cancelled.' }

    const po = procPurchaseOrders.find((o) => o.id === grn.purchaseOrderId)
    if (!po) return { ok: false, error: 'Linked purchase order not found.' }
    if (po.status === 'draft' || po.status === 'pending_approval' || po.status === 'rejected') {
      return { ok: false, error: 'Goods can only be received against an approved purchase order.' }
    }

    const accepted = grn.lines.filter((l) => (Number(l.quantityReceiving) || 0) > 0)
    if (!accepted.length) return { ok: false, error: 'Enter a quantity to receive on at least one line.' }

    // Over-receipt guard: never accept more than the outstanding balance.
    for (const line of accepted) {
      const ordered = po.lines.filter((l) => l.itemId === line.itemId).reduce((sum, l) => sum + l.quantity, 0)
      if (ordered <= 0) return { ok: false, error: 'A received line is not on the purchase order.' }
      const already = receivedQty(goodsReceipts, po.id, line.itemId, grn.id)
      if (already + line.quantityReceiving > ordered) {
        const item = inventory.find((i) => i.id === line.itemId)
        return {
          ok: false,
          error: `Cannot receive ${line.quantityReceiving} of ${item?.name || line.itemId} — only ${Math.max(0, ordered - already)} outstanding on ${po.number}.`,
        }
      }
    }

    // Move stock. Accepted quantity only; rejected units never enter inventory.
    for (const line of accepted) {
      adjustStock(line.itemId, 'in', line.quantityReceiving, `Goods receipt ${grn.number} against ${po.number}`)
    }

    const postedAt = new Date().toISOString()
    const postedGrn: GoodsReceipt = { ...grn, status: 'posted', postedAt, postedBy: userName || 'system' }
    // Stock has moved, so raise the matching ledger entry: Dr Inventory / Cr GRNI.
    syncHistory('goods_receipt', postedGrn.id, historyRowsForGoodsReceipt(postedGrn, {
      supplierName: suppliersRef.current.find((x) => x.id === postedGrn.supplierId)?.name,
    }))
    setGoodsReceiptsState((s) => {
      // Append when the pending record has not landed in state yet.
      const n = s.some((x) => x.id === grn.id)
        ? s.map((x) => (x.id === grn.id ? postedGrn : x))
        : [...s, postedGrn]
      saveGoodsReceipts(n)
      // Advance the purchase order using the freshly posted receipt list.
      const next = nextPoStatus(po, n)
      if (next !== po.status) {
        setProcPurchaseOrdersState((ps) => {
          const pn = ps.map((x) => (x.id === po.id ? { ...x, status: next } : x))
          saveProcPurchaseOrders(pn); return pn
        })
      }
      return n
    })
    return { ok: true }
  }, [goodsReceipts, procPurchaseOrders, inventory, adjustStock, syncHistory])

  /* ── Procurement phase 2: invoices, payments, returns ─────────────────── */

  const upsertSupplierInvoice = useCallback((i: SupplierInvoice) => {
    const scoped = stampSelectedOrg(i)
    // Mirror the invoice into the general ledger. Posting replaces any earlier
    // rows; reverting to draft or cancelling clears them.
    syncHistory('supplier_invoice', scoped.id, historyRowsForSupplierInvoice(scoped, {
      supplierName: suppliersRef.current.find((x) => x.id === scoped.supplierId)?.name,
    }))
    setSupplierInvoicesState((s) => {
      const n = s.some((x) => x.id === scoped.id) ? s.map((x) => (x.id === scoped.id ? scoped : x)) : [...s, scoped]
      saveSupplierInvoices(n); return n
    })
  }, [stampSelectedOrg, syncHistory])

  const deleteSupplierInvoice = useCallback((id: string) => {
    syncHistory('supplier_invoice', id, [])
    setSupplierInvoicesState((s) => { const n = s.filter((x) => x.id !== id); saveSupplierInvoices(n); return n })
  }, [syncHistory])

  /**
   * Save a payment, then re-derive the parent invoice's status so
   * posted/partially paid/paid always reflects the posted payments.
   */
  const upsertSupplierPayment = useCallback((p: SupplierPayment) => {
    const scoped = stampSelectedOrg(p)
    // Mirror the payment into the general ledger (Account History). Posting
    // replaces any previous rows; reverting to draft/cancelled clears them.
    syncHistory('supplier_payment', scoped.id, historyRowsForSupplierPayment(scoped, {
      supplierName: suppliersRef.current.find((x) => x.id === scoped.supplierId)?.name,
    }))
    setSupplierPaymentsState((s) => {
      const n = s.some((x) => x.id === scoped.id) ? s.map((x) => (x.id === scoped.id ? scoped : x)) : [...s, scoped]
      saveSupplierPayments(n)
      // A payment may settle several invoices — refresh the status of each.
      setSupplierInvoicesState((inv) => {
        const ids = new Set(paymentInvoiceIds(scoped))
        if (!ids.size) return inv
        let changed = false
        const updated = inv.map((x) => {
          if (!ids.has(x.id)) return x
          const next = invoicePayStatus(x, n)
          if (next === x.status) return x
          changed = true
          return { ...x, status: next }
        })
        if (!changed) return inv
        saveSupplierInvoices(updated); return updated
      })
      return n
    })
  }, [stampSelectedOrg, syncHistory])
  ...
  const deleteSupplierPayment = useCallback((id: string) => {
    syncHistory('supplier_payment', id, [])
    setSupplierPaymentsState((s) => {
      ...
      if (removed) {
        setSupplierInvoicesState((inv) => {
          const ids = new Set(paymentInvoiceIds(removed))
          if (!ids.size) return inv
          let changed = false
          const updated = inv.map((x) => {
            if (!ids.has(x.id)) return x
  ...
  const upsertProcReturn = useCallback((r: ProcPurchaseReturn) => {
    const scoped = stampSelectedOrg(r)
    setProcReturnsState((s) => {
      const n = s.some((x) => x.id === scoped.id) ? s.map((x) => (x.id === scoped.id ? scoped : x)) : [...s, scoped]
      saveProcReturns(n); return n
    })
  }, [stampSelectedOrg])

  const deleteProcReturn = useCallback((id: string) => {
    setProcReturnsState((s) => { const n = s.filter((x) => x.id !== id); saveProcReturns(n); return n })
  }, [])

  /**
   * Post a purchase return. Mirrors postGoodsReceipt in reverse: validates the
   * quantities against the source receipt, then REDUCES stock. This is the only
   * procurement path that takes goods back out of inventory.
   */
  const postProcReturn = useCallback((id: string, userName?: string, pending?: ProcPurchaseReturn) => {
    const ret = (pending && pending.id === id) ? pending : procReturns.find((r) => r.id === id)
    if (!ret) return { ok: false, error: 'Return not found.' }
    if (ret.status === 'returned' || ret.status === 'closed') return { ok: false, error: 'This return has already been posted.' }
    if (ret.status === 'cancelled') return { ok: false, error: 'This return has been cancelled.' }

    const grn = goodsReceipts.find((g) => g.id === ret.goodsReceiptId)
    if (!grn) return { ok: false, error: 'Linked goods receipt not found.' }
    if (grn.status !== 'posted') return { ok: false, error: 'Goods can only be returned against a posted receipt.' }

    const lines = ret.lines.filter((l) => (Number(l.quantityReturned) || 0) > 0)
    if (!lines.length) return { ok: false, error: 'Enter a quantity to return on at least one line.' }

    // Never return more than was accepted, net of earlier returns.
    for (const line of lines) {
      const received = grn.lines
        .filter((l) => l.itemId === line.itemId)
        .reduce((s, l) => s + l.quantityReceiving, 0)
      const already = returnedQty(procReturns, grn.id, line.itemId, ret.id)
      if (already + line.quantityReturned > received) {
        const item = inventory.find((i) => i.id === line.itemId)
        return {
          ok: false,
          error: `Cannot return ${line.quantityReturned} of ${item?.name || line.itemId} — only ${Math.max(0, received - already)} available on ${grn.number}.`,
        }
      }
      // Guard against sending stock negative if it was consumed after receipt.
      const onHand = inventory.find((i) => i.id === line.itemId)?.quantity ?? 0
      if (line.quantityReturned > onHand) {
        const item = inventory.find((i) => i.id === line.itemId)
        return { ok: false, error: `Only ${onHand} of ${item?.name || line.itemId} in stock — cannot return ${line.quantityReturned}.` }
      }
    }

    for (const line of lines) {
      adjustStock(line.itemId, 'out', line.quantityReturned, `Purchase return ${ret.number} against ${grn.number}`)
    }

    const postedRet: ProcPurchaseReturn = {
      ...ret, status: 'returned', postedAt: new Date().toISOString(), postedBy: userName || 'system',
    }
    setProcReturnsState((s) => {
      const n = s.some((x) => x.id === ret.id)
        ? s.map((x) => (x.id === ret.id ? postedRet : x))
        : [...s, postedRet]
      saveProcReturns(n); return n
    })
    return { ok: true }
  }, [procReturns, goodsReceipts, inventory, adjustStock])
  ...
  // `syncHistory` is declared with the account-history state above so that
  // earlier posting callbacks (e.g. supplier payments) can reference it.
  ...
  const upsertJournalTemplateBase = makeUpsert<JournalTemplate>(setJournalTemplatesState, saveJournalTemplates)
  const upsertJournalTemplate = useCallback((t: JournalTemplate) => {
    upsertJournalTemplateBase(stampSelectedOrg(t))
  }, [stampSelectedOrg, upsertJournalTemplateBase])
  const deleteJournalTemplateBase = makeDelete<JournalTemplate>(setJournalTemplatesState, saveJournalTemplates)
  const deleteJournalTemplate = useCallback((id: string) => deleteJournalTemplateBase(id), [deleteJournalTemplateBase])
  ...
  const recordSale = useCallback((input: SaleInput) => {
    const lines = input.lines.filter((l) => l.itemId && l.quantity > 0)
    if (!lines.length) return { ok: false, error: 'Add at least one item.' }
    // Validate stock (non-inventory items are unlimited and never validated).
    for (const l of lines) {
      const item = inventory.find((i) => i.id === l.itemId)
      if (!item) return { ok: false, error: 'Item not found.' }
      if (!item.nonInventory && item.quantity < l.quantity) return { ok: false, error: `Not enough stock of "${item.name}" (${item.quantity} left).` }
    }
    ...
    setInventoryState((s) => {
      const next = s.map((item) => {
        const line = lines.find((l) => l.itemId === item.id)
        // Non-inventory items are not stock-tracked — never decrement them.
        if (!line || item.nonInventory) return item
    ...
    setStockMovementsState((s) => {
      const movements: StockTransaction[] = lines.filter((l) => !inventory.find((i) => i.id === l.itemId)?.nonInventory).map((l) => ({
        id: uid('sm'),
  ...
  const resetData = useCallback((keys: string[]) => {
    ...
    if (want.has('invoices')) { setInvoices(INVOICES); setInvoiceTemplates(INVOICE_TEMPLATES); rm(INV_KEY); rm(INVOICE_TEMPLATES_KEY) }
    ...
    if (want.has('accounting')) {
      ...
      setReceiptsState(RECEIPTS); setPaymentVouchersState(PAYMENT_VOUCHERS); setJournalsState(JOURNALS); setJournalTemplatesState(JOURNAL_TEMPLATES)
      ...
      rm(ACCOUNTS_KEY); rm(ACCT_SETTINGS_KEY); rm(RECEIPTS_KEY); rm(PAYMENTS_KEY); rm(JOURNALS_KEY); rm(JOURNAL_TEMPLATES_KEY)
  ...
  }, [assets, branches, classes, customers, estimates, inventory, invoices, invoiceTemplates, jobs, journals, journalTemplates, members, paymentVouchers, proposals, purchaseOrders, purchases, purchaseReturns, receipts, sales, salesOrders, staff, suppliers, trainers, users])
  ...
  const value = useMemo<AppStore>(
    () => ({
      ...
      invoiceTemplates: scopeOrgRecords(invoiceTemplates, (t) => ({ userId: t.createdBy })),
      ...
      costCenters: scopeOrgRecords(costCenters),
      ...
      messageTemplates: scopeCompanyRecords(messageTemplates),
      ...
      journalTemplates: scopeOrgRecords(journalTemplates, (t) => ({ userId: t.createdBy })),
      ...
      supplierInvoices, supplierPayments, procReturns, upsertSupplierInvoice, deleteSupplierInvoice, upsertSupplierPayment, deleteSupplierPayment, upsertProcReturn, deleteProcReturn, postProcReturn, requisitions, procPurchaseOrders, goodsReceipts, upsertRequisition, deleteRequisition, upsertProcPurchaseOrder, deleteProcPurchaseOrder, upsertGoodsReceipt, deleteGoodsReceipt, postGoodsReceipt, upsertInventoryItem, deleteInventoryItem, adjustStock, updatePurchase, purchasePayments, addPurchasePayment, updatePurchasePayment, deletePurchasePayment,
      ...
      upsertInvoiceTemplate: (t) => { const scoped = stampSelectedOrg(t); setInvoiceTemplates((s) => (s.some((x) => x.id === scoped.id) ? s.map((x) => (x.id === scoped.id ? scoped : x)) : [scoped, ...s])) },
      deleteInvoiceTemplate: (id) => setInvoiceTemplates((s) => s.filter((t) => t.id !== id)),
      ...
      upsertMessageTemplate: (t) => { const scoped = stampSelectedOrg(t); setMessageTemplatesState((s) => { const n = s.some((x) => x.id === scoped.id) ? s.map((x) => (x.id === scoped.id ? scoped : x)) : [...s, scoped]; saveMessageTemplates(n); return n }) },
      deleteMessageTemplate: (id) => setMessageTemplatesState((s) => { const n = s.filter((t) => t.id !== id); saveMessageTemplates(n); return n }),
      ...
      upsertReceipt, deleteReceipt, upsertPaymentVoucher, deletePaymentVoucher, upsertJournal, deleteJournal, upsertJournalTemplate, deleteJournalTemplate,
      ...
      upsertCostCenter: (c) => setCostCenters((s) => (s.some((x) => x.id === c.id) ? s.map((x) => (x.id === c.id ? c : x)) : [...s, c])),
      deleteCostCenter: (id) => setCostCenters((s) => s.filter((c) => c.id !== id)),
    }),
    [
      ...
      attendance, workouts, progress, notifications, branches, costCenters, leads, messages, audit, leaves, sessions, company,
      ...
      cms, setCms,
      upsertCompany, deleteCompany, setCompanyStatus, setActiveCompany, setActiveBranch, setProductMode,
      inventory, suppliers, stockMovements, stockTransfers, stockAdjustments, stockCounts, stockAlerts, inventoryCategories, purchases, sales, purchaseOrders, purchaseReturns, shipments, discounts, salesReturns, departments, payslips, jobs, candidates, reviews, staffAttendance, assets, depreciation, assetTransactions, assetCategories, assetConditions, depreciationPolicy, customers, supplierCategories, customerCategories, modules, sidebarOrder, accounts, accountingSettings, receipts, paymentVouchers, journals, journalTemplates, banks, signatories, voucherSerials, funds, paymentModes, detailTypes, incomeMods, currencyRates, reconciliations, budgets, valueBook, cheques, proposals, estimates, salesOrders,
      supplierInvoices, supplierPayments, procReturns, upsertSupplierInvoice, deleteSupplierInvoice, upsertSupplierPayment, deleteSupplierPayment, upsertProcReturn, deleteProcReturn, postProcReturn, requisitions, procPurchaseOrders, goodsReceipts, upsertRequisition, deleteRequisition, upsertProcPurchaseOrder, deleteProcPurchaseOrder, upsertGoodsReceipt, deleteGoodsReceipt, postGoodsReceipt, scopeStockTransfers, upsertInventoryItem, deleteInventoryItem, adjustStock, updatePurchase, purchasePayments, addPurchasePayment, updatePurchasePayment, deletePurchasePayment, upsertStockTransfer, deleteStockTransfer, upsertStockAdjustment, deleteStockAdjustment, upsertStockCount, deleteStockCount, upsertStockAlert, deleteStockAlert, upsertSupplier, deleteSupplier,
      addInventoryCategory, renameInventoryCategory, deleteInventoryCategory,
      ...
      upsertAccount, deleteAccount, setAccountingSettings, upsertReceipt, deleteReceipt, upsertPaymentVoucher, deletePaymentVoucher, upsertJournal, deleteJournal, upsertJournalTemplate, deleteJournalTemplate,
