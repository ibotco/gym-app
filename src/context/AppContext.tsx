...
import type {
  ...
  StockTransfer, StockAdjustment, StockAlert, StockTransferStatus, StockAdjustmentType,
...
interface AppStore {
  ...
  recordPurchase: (input: { companyId?: string; branchId?: string; supplierId: string; lines: PurchaseLine[]; status?: PurchaseStatus; notes?: string; userId: string; date?: string }) => { ok: boolean; error?: string; purchase?: Purchase }
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const stampSelectedOrg = useCallback(<T extends object>(record: T): T => {
    ...
    const branchId = branchIsPermitted
  ...
  const recordPurchase = useCallback((input: { companyId?: string; branchId?: string; supplierId: string; lines: PurchaseLine[]; status?: PurchaseStatus; notes?: string; userId: string; date?: string }) => {    if (!input.supplierId) return { ok: false, error: 'Select a supplier.' }
  ...
  const scopeOrgRecords = useCallback(function scopeOrgRecords<T extends object>(
    ...
    return records.filter((record) => {
      ...
      if (respectActiveBranch && selectedBranchScopeId && org.branchId && org.branchId !== selectedBranchScopeId) return false
      return true
    })
  ...
  const value = useMemo<AppStore>(
    () => ({
      ...
      upsertStockTransfer, deleteStockTransfer, upsertStockAdjustment, deleteStockAdjustment, upsertStockAlert, deleteStockAlert,
