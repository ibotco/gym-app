...
export function Home() {
  ...
  const { upsertLead, company, cms } = useApp()
  const nav = useNavigate()
  const { t } = useI18n()

  // Front CMS drives the hero, program cards and testimonials.
  const fe = company.frontendCms
  const heroLine1 = fe?.heroHeadline?.trim() || t('home.hero1')
  const heroLine2 = fe?.heroHighlight?.trim() || t('home.hero2')
  const heroSub = fe?.heroSubheadline?.trim() || t('home.heroSub')
  const heroCta = fe?.heroCtaText?.trim() || t('viewPlans')
  const heroImage = fe?.heroImage || (cms.sliders[0] as { desktopImage?: string } | undefined)?.desktopImage || '/images/hero.jpg'
  const programs = cms.services.length
    ? cms.services.map((s) => ({ img: s.image || '/images/program-pt.jpg', t: s.title, d: s.description }))
    : [
  ...
  const testimonials = cms.testimonials.length
    ? cms.testimonials.map((s) => ({ id: s.id, avatar: s.photo || '/images/success-1.jpg', name: s.name, role: s.position, quote: s.text, result: s.organization || 'Verified member' }))
    : TESTIMONIALS

  return (
    <div>
      <section className="relative min-h-[92vh] overflow-hidden">
        <img src={heroImage} alt="FitPro training floor" className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="relative mx-auto flex min-h-[92vh] max-w-7xl flex-col justify-end px-4 pb-16 pt-28 md:px-6 md:pb-24">
          <motion.div initial="hidden" animate="show" variants={fade} className="max-w-3xl">
            ...
            <h1 className="font-display text-5xl font-semibold leading-[0.95] tracking-tight text-white md:text-7xl">
              {heroLine1}
              <br />
              <span className="text-lime">{heroLine2}</span>
            </h1>
            <p className="mt-5 max-w-xl text-lg text-zinc-300">{heroSub}</p>
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <Link to="/membership"><Button size="lg" variant="ghost">{heroCta}</Button></Link>
            </div>
