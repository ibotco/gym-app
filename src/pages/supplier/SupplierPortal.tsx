...
export function SupplierPortal() {
  const { user } = useAuth()
  const { suppliers, purchaseOrders, purchases, inventory, branches } = useApp()
  const supplier = suppliers.find((s) => s.userId === user?.id)
  const assignedBranch = branches.find((branch) => branch.id === (supplier?.branchId || user?.branchId))
  ...
  return (
    <div>
      <PageHeader
        ...
        desc={supplier ? `${supplier.category || 'Supplier'} · ${supplier.email} · ${assignedBranch?.name || 'Branch not assigned'}` : 'Supplier account'}
      />
      ...
      <div className="card mt-4 p-5">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          {[
            ...
            ['Home branch', assignedBranch?.name || 'Branch not assigned'],
