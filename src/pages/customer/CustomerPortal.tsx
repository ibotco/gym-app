...
export function CustomerPortal() {
  const { user } = useAuth()
  const { customers, invoices, sales, branches } = useApp()
  const customer = customers.find((c) => c.userId === user?.id)
  const assignedBranch = branches.find((branch) => branch.id === (customer?.branchId || user?.branchId))
  ...
  return (
    <div>
      <PageHeader
        ...
        desc={customer ? `${customer.category} · ${customer.company || 'Individual customer'} · ${assignedBranch?.name || 'Branch not assigned'}` : 'Customer account'}
      />
