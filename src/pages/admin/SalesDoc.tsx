  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Button onClick={openNew}><Plus className="size-4" /> {isEstimate ? 'Add Estimate' : `New ${label.toLowerCase()}`}</Button>}
      />
      ...
      {/* Add / edit */}
      <Modal open={!!editing} onClose={() => setEditing(null)} title={editing?.id ? `Edit ${isEstimate ? 'Estimate' : label.toLowerCase()}` : `New ${isEstimate ? 'Estimate' : label.toLowerCase()}`} wide>
        {editing && (
          <div className="space-y-3">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={save}><Send className="size-4" /> {editing.id ? `Save ${isEstimate ? 'Estimate' : label.toLowerCase()}` : isEstimate ? 'Add Estimate' : `Create ${label.toLowerCase()}`}</Button>
            </div>
          </div>
        )}
      </Modal>
