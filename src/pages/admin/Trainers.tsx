...
const blank = (): TrainerForm => ({
  staffId: '', branchId: '', specialties: '', certifications: '', experienceYears: '1',
  hourlyRate: '150', bio: '', rating: '5', clientsCount: '0', showOnWebsite: true,
})

export function TrainersAdmin() {
  const app = useApp()
  const { trainers, users, staff, branches, activeBranchId, upsertTrainer, upsertStaff, patchUser, deleteTrainer, log } = app
  ...
  const openCreate = () => { setEditing({ ...blank(), branchId: activeBranchId || branchOptions[0]?.id || '' }); setErr('') }

  const openEdit = (t: Trainer) => {
    setEditing({
      id: t.id,
      staffId: staff.find((s) => s.userId === t.userId)?.id || '',
      branchId: t.branchId || staff.find((s) => s.userId === t.userId)?.branchId || users.find((u) => u.id === t.userId)?.branchId || activeBranchId || branchOptions[0]?.id || '',
  ...
  const save = () => {
    ...
    if (!editing.branchId || !branchOptions.some((branch) => branch.id === editing.branchId)) { setErr('Select a permitted home branch.'); return }
    ...
    const assignedBranch = branches.find((branch) => branch.id === editing.branchId)
    const record: Trainer = {
      id: existing?.id || uid('tr'),
      companyId: assignedBranch?.companyId || existing?.companyId,
    ...
    upsertStaff({ ...rec, companyId: assignedBranch?.companyId || rec.companyId, branchId: editing.branchId })
    patchUser(rec.userId, { companyId: assignedBranch?.companyId, branchId: editing.branchId })
  ...
  const columns: Column<{ t: Trainer; name: string; photo?: string; title?: string; department?: string }>[] = [
    ...
    { key: 'branch', header: 'Branch', sortValue: (r) => branches.find((branch) => branch.id === r.t.branchId)?.name || '', render: (r) => branches.find((branch) => branch.id === r.t.branchId)?.name || '—' },
