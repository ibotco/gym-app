...
type FieldDef = {
  ...
  label: string
  kind: 'text' | 'select' | 'color' | 'weekends'
...
export function BranchSettingsPanel() {
  ...
  const saveCurrency = () => {
    ...
    setCurrencies((list): BranchCurrency[] => {
      ...
      if (idx === null) {
        // If setting this new currency as base, demote the existing base.
        if (entry.status === 'base') return [...list.map((c): BranchCurrency => (c.status === 'base' ? { ...c, status: 'alternate' } : c)), entry]
        return [...list, entry]
      }
      // Editing: enforce exactly one base currency.
      return list.map((c, i): BranchCurrency => {
  ...
  const removeCurrency = (i: number) => setCurrencies((list) => {
    const next = list.filter((_, idx) => idx !== i)
    // If the removed currency was the base, promote the first remaining one.
    if (list[i]?.status === 'base' && next.length && !next.some((c) => c.status === 'base')) {
      return next.map((c, idx) => (idx === 0 ? { ...c, status: 'base' } : c))
    }
    return next
  })
  const setBaseCurrency = (i: number) => setCurrencies((list) => list.map((c, idx) => ({ ...c, status: idx === i ? 'base' : 'alternate' })))
  ...
  const saveTax = () => {
    ...
    setTaxRates((list) => {
      if (taxModal.index === null) return [...list, entry]
      return list.map((t, i) => (i === taxModal.index ? entry : t))
    })
    setTaxModal(null)
  }
  const removeTax = (i: number) => setTaxRates((list) => list.filter((_, idx) => idx !== i))
  const toggleTaxStatus = (i: number) => setTaxRates((list) => list.map((t, idx) => (idx === i ? { ...t, status: t.status === 'active' ? 'inactive' : 'active' } : t)))
