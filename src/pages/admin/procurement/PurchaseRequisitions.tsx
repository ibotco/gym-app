...
type LineDraft = { itemId: string; qty: string; estCost: string }
...
export function PurchaseRequisitions() {
  const {
    requisitions, procPurchaseOrders, inventory, branches, suppliers,
  ...
  const openEdit = (r: PurchaseRequisition) => setEditing({
    ...
    lines: r.lines.map((l) => ({ itemId: l.itemId, qty: String(l.quantity), estCost: String(l.estimatedCost ?? '') })),
  })
  ...
  const save = (submit: boolean) => {
    ...
    const lines: RequisitionLine[] = editing.lines
      .map((l): RequisitionLine | null => {
        ...
        return { itemId: l.itemId, quantity, estimatedCost: Number(l.estCost) || undefined }
      })
        ...
        {editing && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="table-wrap">
                <table className="data">
                  <thead><tr><th>Item</th><th className="w-28">Quantity</th><th className="w-36">Est. unit cost</th><th className="w-32 text-right">Est. total</th><th /></tr></thead>
                  <tbody>
