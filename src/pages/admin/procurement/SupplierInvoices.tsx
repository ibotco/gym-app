...
import {
  ...
  fullInvoiceTotals, validateInvoice, invoiceEditable, invoiceDeletable, isPostableExpenseAccount, invoiceJournalPreview,
...
type LineDraft = {
  ...
  discount: string; tax: string; costCenterId?: string
}
type ExpenseDraft = {
...
export function SupplierInvoices() {
  const {
    ...
    inventory, suppliers, branches, activeBranchId, accounts, company, branchSettings, costCenters, upsertSupplierInvoice, deleteSupplierInvoice, log,
  } = useApp()
  ...
  const [editing, setEditing] = useState<
    | { id?: string; number: string; supplierId: string; purchaseOrderId: string; goodsReceiptId: string
        ...
        discountId: string; discountType: 'percentage' | 'fixed'; discountValue: string
        taxName: string; taxRate: number }
    | null
  >(null)
  const [tab, setTab] = useState<EditorTab>('items')
  const [showJournal, setShowJournal] = useState(false)
  // Discount & tax panel — collapsible like the Add Sale form. Auto-expands
  ...
  useEffect(() => {
    ...
    if (isEditing && !editorJustOpenedRef.current) {
      setDiscountTaxOpen(Boolean(editing.discountId || (Number(editing.discountValue) || 0) > 0 || !!editing.taxName))
    }
    editorJustOpenedRef.current = isEditing
  }, [editing])
  ...
  /** POs that can be offered for invoicing — any status except rejected/cancelled.
      Numbers already used in the Purchase Invoices list are filtered out below. */
  const offerablePos = useMemo(
    () => procPurchaseOrders.filter((o) => !['rejected', 'cancelled'].includes(o.status)),
    [procPurchaseOrders],
  )

  /** PO / GRN numbers already appearing in the Purchase Invoices list. */
  const usedPurchaseOrderIds = useMemo(
    () => new Set(supplierInvoices.map((i) => i.purchaseOrderId).filter((id): id is string => !!id)),
    [supplierInvoices],
  )
  const usedGoodsReceiptIds = useMemo(
    () => new Set(supplierInvoices.map((i) => i.goodsReceiptId).filter((id): id is string => !!id)),
    [supplierInvoices],
  )
  /** Posted receipts not yet invoiced — already-used GRN numbers are hidden from the source picker. */
  const newPostedGrns = postedGrns.filter((g) => !usedGoodsReceiptIds.has(g.id))
  /** POs not yet invoiced — already-used PO numbers are hidden from the create flow. */
  const newInvoiceablePos = offerablePos.filter((o) => !usedPurchaseOrderIds.has(o.id))

  /** Reference-picker options. Create flow: only documents that are NOT already shown
      in the Purchase Invoices list (plus the currently selected one, so a link
      pre-filled from the source picker stays visible). Edit flow: the full list,
      so an existing linkage can be reviewed or changed. */
  const createPoOptions = (() => {
    if (!editing) return []
    const sel = editing.purchaseOrderId ? procPurchaseOrders.find((o) => o.id === editing.purchaseOrderId) : undefined
    return sel && !newInvoiceablePos.some((o) => o.id === sel.id) ? [sel, ...newInvoiceablePos] : newInvoiceablePos
  })()
  const createGrnOptions = (() => {
    if (!editing) return []
    const sel = editing.goodsReceiptId ? goodsReceipts.find((g) => g.id === editing.goodsReceiptId) : undefined
    return sel && !newPostedGrns.some((g) => g.id === sel.id) ? [sel, ...newPostedGrns] : newPostedGrns
  })()
  const poOptions = editing?.id ? procPurchaseOrders : createPoOptions
  const grnOptions = editing?.id ? postedGrns : createGrnOptions
  ...
  const blankEditor = () => ({
    ...
    discountId: '', discountType: 'percentage' as const, discountValue: '0',
    taxName: '', taxRate: 0,
  })
  ...
  /** Taxes configured for the invoice's business location (fallback: defaults).
      A previously saved tax stays visible while editing if it was later disabled. */
  const taxOptions = useMemo(() => {
    const settings = branchSettingsFor(branchSettings, editing?.branchId || activeBranchId || '')
    ...
    if (editing?.taxName && !activeTaxes.some((t) => t.name === editing.taxName)) {
      return [...activeTaxes, { name: editing.taxName, rate: editing.taxRate || 0, status: 'active' as const }]
    }
    return activeTaxes
  }, [branchSettings, editing?.branchId, editing?.taxName, editing?.taxRate, activeBranchId])

  /** Live totals across both tabs, including the order-level discount & tax.
      A picked code wins; otherwise the manual value is used. */
  const draftTotals = useMemo(() => {
    const empty = { itemGross: 0, discountTotal: 0, itemSubtotal: 0, expenseSubtotal: 0, itemTax: 0, expenseTax: 0, orderDiscount: 0, orderTax: 0, taxTotal: 0, total: 0 }
    if (!editing) return empty
    const lineMapped = editing.lines.map((l) => ({
      quantity: Number(l.qty) || 0, unitCost: Number(l.unitCost) || 0,
      discountPercent: Number(l.discount) || 0, taxRate: Number(l.tax) || 0,
    }))
    const expenseMapped = editing.expenses.map((e) => ({ amount: Number(e.amount) || 0, taxRate: Number(e.tax) || 0 }))
    const baseTotals = fullInvoiceTotals(lineMapped, expenseMapped)
    const orderBase = baseTotals.itemSubtotal + baseTotals.expenseSubtotal
    const picked = tableDiscounts.find((d) => d.id === editing.discountId)
    const itemIds = editing.lines.map((l) => l.itemId).filter(Boolean)
    const orderDiscount = picked
      ? computeDiscount(picked, orderBase, itemIds)
      : Math.min(orderBase, editing.discountType === 'percentage'
          ? (orderBase * (Math.max(0, Number(editing.discountValue) || 0))) / 100
          : Math.max(0, Number(editing.discountValue) || 0))
    return fullInvoiceTotals(lineMapped, expenseMapped, { discountAmount: orderDiscount, taxRate: editing.taxRate || 0 })
  }, [editing, tableDiscounts])

  /** Applied value of the picked code, and why it might not apply. */
  const codeDiscountValue = selectedDiscount && editing
    ? computeDiscount(selectedDiscount, draftTotals.itemSubtotal + draftTotals.expenseSubtotal, editing.lines.map((l) => l.itemId).filter(Boolean))
    : null
  const codeBlock = selectedDiscount && editing
    ? discountBlockReason(selectedDiscount, draftTotals.itemSubtotal + draftTotals.expenseSubtotal, {
        itemIds: editing.lines.map((l) => l.itemId).filter(Boolean),
        productName: (id: string) => inventory.find((i) => i.id === id)?.name,
      })
    : null
  ...
  const journalPreview = useMemo(() => {
    ...
    return invoiceJournalPreview(
      ...
      accounts,
      { orderDiscount: draftTotals.orderDiscount, orderTax: draftTotals.orderTax },
    )
  }, [editing, accounts, draftTotals])
  ...
  const save = (mode: SaveMode) => {
    ...
    const code = editing.discountId ? tableDiscounts.find((d) => d.id === editing.discountId) : undefined
    const lineTotals = fullInvoiceTotals(lines, expenseLines)
    const orderBase = lineTotals.itemSubtotal + lineTotals.expenseSubtotal
    const resolvedOrderDiscount = code
      ? computeDiscount(code, orderBase, lines.map((l) => l.itemId))
      : Math.min(orderBase, editing.discountType === 'percentage'
          ? (orderBase * (Math.max(0, Number(editing.discountValue) || 0))) / 100
          : Math.max(0, Number(editing.discountValue) || 0))
    const codeApplied = code && resolvedOrderDiscount > 0 ? code : undefined
    const t = fullInvoiceTotals(lines, expenseLines, { discountAmount: resolvedOrderDiscount, taxRate: editing.taxRate || 0 })
    const existing = supplierInvoices.find((i) => i.id === editing.id)
    const isNew = !editing.id
    const now = new Date().toISOString()
    const who = user?.name || 'system'

    // Only move the status forward; a posted invoice never regresses.
    const nextStatus: SupplierInvoice['status'] =
      existing?.status === 'posted' || mode === 'post' ? 'posted'
    ...
    const rec: SupplierInvoice = {
      ...
      discountId: codeApplied?.id,
      discountCode: codeApplied?.code,
      discountName: codeApplied?.name,
      discountType: editing.discountType,
      discountValue: Number(editing.discountValue) || 0,
      orderDiscount: t.orderDiscount,
      taxName: editing.taxName || undefined,
      taxRate: editing.taxRate || undefined,
      orderTax: t.orderTax,
      ...
      postedAt: mode === 'post' ? (existing?.postedAt || now) : existing?.postedAt,
      postedBy: mode === 'post' ? (existing?.postedBy || who) : existing?.postedBy,
    ...
    <div>
      <PageHeader
        ...
        actions={canManage ? (
          <Button
            onClick={() => {
              // Nothing left to pick from (every source is already invoiced) — skip straight to the New Purchase Invoice editor.
              if (newPostedGrns.length === 0 && newInvoiceablePos.length === 0) { setEditing(blankEditor()); setTab('items') }
              else setSourcePick(true)
            }}
          >
                  ...
                  <tr key={i.id} style={{ background: idx % 2 ? ROW_ALT : 'transparent' }}>
                    ...
                    {visibleCols.has('action') && (
                      <td className="whitespace-nowrap px-3 py-2.5" style={{ borderBottom: `1px solid ${PANEL_BD}` }}>
                        ...
                        {canManage && invoiceDeletable(i.status) && (
      ...
      <Modal open={sourcePick} onClose={() => setSourcePick(false)} title="Create purchase invoice" wide>
        <div className="space-y-4">
          ...
          <div>
            ...
            {newPostedGrns.length ? (
              <div className="table-wrap">
                <table className="data">
                  {/* GRN / PO numbers are already shown in the list columns, so they are omitted here.
                      Receipts already invoiced (shown in the list) are not offered again. */}
                  <thead><tr><th>Date</th><th>Supplier</th><th className="text-right">Value</th><th /></tr></thead>
                  <tbody>
                    {newPostedGrns.map((g) => (
                      <tr key={g.id}>
            ...
            ) : <p className="text-xs text-mist">{postedGrns.length ? 'All posted goods receipts are already invoiced.' : 'No posted goods receipts yet.'}</p>}
          </div>

          <div>
            <SubHead>From a purchase order</SubHead>
            {newInvoiceablePos.length ? (
              <div className="table-wrap">
                <table className="data">
                  {/* PO numbers are already shown in the list columns, so they are omitted here.
                      Orders already invoiced (shown in the list) are not offered again. */}
                  <thead><tr><th>Date</th><th>Supplier</th><th>Status</th><th className="text-right">Total</th><th /></tr></thead>
                  <tbody>
                    {newInvoiceablePos.map((o) => (
                      <tr key={o.id}>
            ...
            ) : <p className="text-xs text-mist">{offerablePos.length ? 'All purchase orders are already invoiced.' : 'No purchase orders yet.'}</p>}
          </div>
              ...
              <div id="sinv-print" className="rounded-xl bg-white p-5 text-sm text-zinc-900">
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="bg-zinc-100 text-left uppercase tracking-wide text-zinc-500">
                        ...
                        {showCostCenter && <th className="px-3 py-2">Cost center</th>}
                      ...
                      {open.lines.map((l, i) => {
                        ...
                        return (
                          <tr key={i} className="border-t border-zinc-100">
                            ...
                            {showCostCenter && <td className="px-3 py-2">{costCenterName(costCenters, l.costCenterId)}</td>}
                ...
                <div className="mt-3 flex items-end justify-between">
                  ...
                  <div className="w-60 space-y-1 text-xs">
                    ...
                    {!!open.orderDiscount && (
                      <div className="flex justify-between"><span>Order discount{open.discountName ? ` — ${open.discountName}` : ''}</span><span>−{formatGhsExact(open.orderDiscount)}</span></div>
                    )}
                    <div className="flex justify-between"><span>Tax</span><span>{formatGhsExact(open.taxTotal)}</span></div>
                    {!!open.orderTax && (
                      <div className="flex justify-between"><span>Order tax{open.taxName ? ` — ${open.taxName}` : ''}</span><span>{formatGhsExact(open.orderTax)}</span></div>
                    )}
                ...
                <div className="card p-4">
                  ...
                  {mine.length ? (
                    <>
                      ...
                      {mine.some((p) => p.status === 'draft') && (
                        <p className="mt-2 text-xs text-mist">Draft (unposted) payments do not reduce the balance until they are posted.</p>
                      )}
                    </>
        ...
        {editing && (
          <div className="space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              {/* PO / GRN reference pickers. In the create flow only numbers that are NOT
                  already shown in the Purchase Invoices list are offered (used numbers
                  stay hidden); the edit flow keeps the full list so an existing
                  linkage can be reviewed or changed. */}
              <Field label="Purchase order reference">
                <Select value={editing.purchaseOrderId} onChange={(e) => setEditing({ ...editing, purchaseOrderId: e.target.value })} placeholder="Search orders…">
                  ...
                  {poOptions.map((o) => <option key={o.id} value={o.id}>{o.number} — {supplierName(o.supplierId)}</option>)}
                </Select>
              </Field>
              <Field label="Goods receipt reference">
                <Select value={editing.goodsReceiptId} onChange={(e) => setEditing({ ...editing, goodsReceiptId: e.target.value })} placeholder="Search receipts…">
                  <option value="">None</option>
                  {grnOptions.map((g) => <option key={g.id} value={g.id}>{g.number} — {supplierName(g.supplierId)}</option>)}
                </Select>
              </Field>
            </div>

            {/* Discount & tax (order level) — optional panel, collapsible like the Add Sale form. */}
                ...
                <div className="border-t border-zinc-500/20 p-3 sm:p-4">
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <Field label="Tax name">
                      <Select value={editing.taxName} onChange={(e) => {
                        const nextName = e.target.value
                        const picked = taxOptions.find((t) => t.name === nextName)
                        setEditing({ ...editing, taxName: nextName, taxRate: nextName ? (picked?.rate || 0) : 0 })
                      }}>
                        <option value="">No tax</option>
                        {taxOptions.map((t) => <option key={t.name} value={t.name}>{t.name}</option>)}
                      </Select>
                    </Field>
                    <Field label="Tax rate (%)">
                      <Input value={`${editing.taxRate}%`} readOnly aria-label="Associated tax rate" />
                    </Field>
                  </div>
                </div>
              )}
                  ...
                  <table className="data line-grid">
                    <thead>
                      <tr>
                        ...
                        {showCostCenter && <th className="w-52">Cost center</th>}
                    ...
                    <tbody>
                      ...
                      {!editing.lines.length && (
                        <tr><td colSpan={showCostCenter ? 9 : 8} className="py-6 text-center text-sm text-mist">No item lines. This invoice can still be saved with expenses only.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
