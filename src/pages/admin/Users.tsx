...
export function UsersAdmin() {
  const { users, members, customers, suppliers, branches, roles, staff, trainers, activeCompanyId, activeBranchId, upsertUser, patchUser, deleteUser, log, upsertMember, upsertCustomer, upsertSupplier, upsertStaff, upsertTrainer, credentialSettings } = useApp()
  const { user: me, hasRole } = useAuth()
  const toast = useToast()
  const branchOptions = visibleBranches(me, branches, activeCompanyId).filter((branch) => branch.status !== 'inactive')
  const definitionCompanyId = me && me.role !== 'super_admin' ? userCompanyId(me, branches) : undefined
  ...
  const openCreate = () => {
    setEditingId(null)
    setForm({ ...blank(), branchId: activeCompanyId && branchOptions.some((branch) => branch.id === activeBranchId) ? activeBranchId : branchOptions[0]?.id || '' })
    setErr('')
    setOpen(true)
  }
  ...
  const openEdit = (u: User) => {
    ...
    setForm({
      ...
      role: u.role,
      status: u.status,
      branchId: u.branchId || (activeBranchId && branchOptions.some((branch) => branch.id === activeBranchId) ? activeBranchId : branchOptions[0]?.id || ''),
      password: '',
    })
    setErr('')
    setOpen(true)
  ...
  const validate = () => {
    ...
    const selectedRole = roles.find((candidate) => candidate.id === form.role)
    if (!selectedRole) return 'That role is not available in your organisation.'
    if (me?.role !== 'super_admin' && !selectedRole.builtin && selectedRole.companyId !== definitionCompanyId) {
      return 'Custom roles can only be assigned within your company.'
    }
    if (form.role === 'super_admin' && !hasRole('super_admin')) {
      return 'Only a super admin can assign the super admin role.'
    }
    if (form.role === 'company_admin' && !hasRole('super_admin', 'gym_manager', 'company_admin')) {
      return 'Only an organisation administrator can assign the company admin role.'
    }
    if (form.role === 'branch_admin' && !hasRole('super_admin', 'gym_manager', 'company_admin', 'branch_admin')) {
      return 'You cannot assign the branch admin role.'
    }
    const branchAssignedRoles: Role[] = ['staff', 'trainer', 'member', 'customer', 'supplier', 'gym_manager', 'receptionist', 'branch_admin', 'company_admin', 'head_office']
    if ((branchAssignedRoles.includes(form.role) || !selectedRole.builtin) && (!form.branchId || !branchOptions.some((branch) => branch.id === form.branchId))) {
      return 'This account role must be assigned to a permitted home branch.'
    }
    if (me?.role === 'branch_admin' && !form.branchId) return 'Branch Admin users must be assigned a branch.'
  ...
  const save = async () => {
    ...
    if (!editingId) {
      ...
      upsertUser({
        id,
        companyId: selectedBranch?.companyId || me?.companyId || activeCompanyId,
        name: form.name.trim(),
      ...
      const selectedBranch = branches.find((branch) => branch.id === form.branchId)
      const companyId = selectedBranch?.companyId || current.companyId || me?.companyId || activeCompanyId
      ...
      const linkedStaff = staff.find((record) => record.userId === current.id)
      if (linkedStaff) upsertStaff({ ...linkedStaff, companyId: selectedBranch?.companyId || companyId, branchId: form.branchId || undefined })
      const linkedTrainer = trainers.find((record) => record.userId === current.id)
      if (linkedTrainer) upsertTrainer({ ...linkedTrainer, companyId: selectedBranch?.companyId || companyId, branchId: form.branchId || undefined })
      const linkedMember = members.find((record) => record.userId === current.id)
      if (linkedMember) upsertMember({ ...linkedMember, companyId: selectedBranch?.companyId || companyId, branchId: form.branchId || undefined })
      const linkedCustomer = customers.find((record) => record.userId === current.id)
      if (linkedCustomer) upsertCustomer({ ...linkedCustomer, companyId: selectedBranch?.companyId || companyId, branchId: form.branchId || undefined })
      const linkedSupplier = suppliers.find((record) => record.userId === current.id)
      if (linkedSupplier) upsertSupplier({ ...linkedSupplier, companyId: selectedBranch?.companyId || companyId, branchId: form.branchId || undefined })
