...
export function ClassesAdmin() {
  const { classes, trainers, users, branches, activeCompanyId, activeBranchId, upsertClass, deleteClass, log } = useApp()
  const { hasRole, user } = useAuth()
  const toast = useToast()
  const canEdit = hasRole('super_admin', 'gym_manager', 'company_admin', 'branch_admin')
  const branchOptions = visibleBranches(user, branches, activeCompanyId).filter((branch) => branch.status !== 'inactive')
  ...
  const columns: Column<GymClass>[] = [
    {
      key: 'name', header: 'Class', sortValue: (c) => c.name,
      render: (c) => (
        <span>
          <span className="block font-semibold">{c.name}</span>
          <span className="block text-xs text-mist">{c.category} · {c.room} · {c.level}</span>
        </span>
      ),
    },
    { key: 'when', header: 'When', sortValue: (c) => `${c.dayOfWeek}-${c.startTime}`, render: (c) => `${WEEKDAYS[c.dayOfWeek]} ${c.startTime}–${c.endTime}` },
    { key: 'coach', header: 'Coach', sortValue: (c) => tname(c.trainerId), render: (c) => tname(c.trainerId) },
    { key: 'club', header: 'Club', sortValue: (c) => branches.find((b) => b.id === c.branchId)?.name || '', render: (c) => <span className="text-mist">{branches.find((b) => b.id === c.branchId)?.name}</span> },
    { key: 'fill', header: 'Fill', sortValue: (c) => c.enrolled / Math.max(1, c.capacity), render: (c) => <Badge tone={c.enrolled >= c.capacity ? 'rose' : 'lime'}>{c.enrolled}/{c.capacity}</Badge> },
    { key: 'wait', header: 'Wait', sortValue: (c) => c.waitlist, align: 'center', render: (c) => c.waitlist },
    {
  ...
  const openCreate = () => {
    setEditingId(null)
    setForm({ ...blank(), trainerId: trainers[0]?.id || '', branchId: activeBranchId || branchOptions[0]?.id || '' })
    setErr('')
    setOpen(true)
  }
  ...
  const validate = () => {
    ...
    if (!form.branchId || !branchOptions.some((branch) => branch.id === form.branchId)) return 'Select a permitted club.'
  ...
  const save = () => {
    ...
    const record: GymClass = {
      id: editingId || uid('cl'),
      companyId: branches.find((branch) => branch.id === form.branchId)?.companyId || activeCompanyId,
      name: form.name.trim(),
    ...
    <div>
      ...
      <div className="card">
        <DataTable
          ...
          emptyDesc="Create your first class with the Create class button."
        />
      </div>
