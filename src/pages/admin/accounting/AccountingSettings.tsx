...
import type { AccountingSettings as AcctSettings, AccountType, VoucherSerial, Fund, PaymentModeOption, AccountDetailType, IncomeStatementMod, CurrencyRate, StakeholderClassDef, StakeholderEntity, Account, AutoMapping } from '../../../types'
...
type MappingTabId = 'general' | 'payslips' | 'purchase' | 'inventory' | 'manufacturing'
...
const GENERAL_LINES: MappingLineDef[] = [
  { section: 'Invoice', key: 'invoice.default', hint: 'Default for all item', leftLabel: 'Payment account', rightLabel: 'Deposit to', hasToggle: true },
  { section: 'Invoice', key: 'invoice.discount', leftLabel: 'Payment account', rightLabel: 'Deposit to', hasToggle: true },
  { section: 'Payment', key: 'payment.sales', leftLabel: 'Payment account', rightLabel: 'Deposit to', hasToggle: true, sectionSub: 'Sales' },
  { section: 'Payment', key: 'payment.expenses', leftLabel: 'Payment account', rightLabel: 'Deposit to', hasToggle: true, sectionSub: 'Expenses' },
  { section: 'Credit Note', key: 'creditnote.sales', leftLabel: 'Payment account', rightLabel: 'Deposit to', hasToggle: true, sectionSub: 'Sales' },
  { section: 'Credit Note', key: 'creditnote.refund', leftLabel: 'Payment account', rightLabel: 'Deposit to', hasToggle: true, sectionSub: 'Refund' },
  { section: 'Expense default', key: 'expense.default', leftLabel: 'Payment account', rightLabel: 'Deposit to', hasToggle: true },
  { section: 'Tax default', key: 'tax.sales', leftLabel: 'Payment account', rightLabel: 'Deposit to', hasToggle: true, sectionSub: 'Sales' },
  { section: 'Tax default', key: 'tax.expenses', leftLabel: 'Payment account', rightLabel: 'Deposit to', hasToggle: true, sectionSub: 'Expenses' },
] as (MappingLineDef & { sectionSub?: string })[]
...
const PURCHASE_LINES: MappingLineDef[] = [
  ...
  { section: 'Payment', key: 'purchase.payment', leftLabel: 'Payment account', rightLabel: 'Deposit to', hasToggle: true },
...
function AutoMappingPanel({
  ...
  const setLine = (key: string, patch: { paymentAccountId?: string; depositToAccountId?: string; enabled?: boolean }) => {
  ...
  // Group general lines by section.
  const grouped = useMemo(() => {
    const out: Record<string, (MappingLineDef & { sectionSub?: string })[]> = {}
    for (const l of GENERAL_LINES) {
      if (!out[l.section]) out[l.section] = []
      out[l.section].push(l)
    }
    return out
  }, [])
  ...
  return (
    <div className="card p-5">
      ...
      <div className="rounded-b-lg border border-t-0 border-line bg-white p-5 dark:bg-white/2">
        ...
        {tab === 'general' && (
          <div className={`space-y-4 ${!mapping.enabled ? 'pointer-events-none opacity-50' : ''}`}>
            {Object.entries(grouped).map(([section, defs]) => (
              <div key={section} className="rounded-md border border-line">
                {defs.map((def, idx) => {
                  const line = (lines[def.key] as { paymentAccountId?: string; depositToAccountId?: string; enabled?: boolean; hint?: string }) ?? {}
                  const enabled = line.enabled !== false
                  return (
                    <div
                      key={def.key}
                      className={`border-line p-4 ${idx > 0 ? 'border-t' : ''}`}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-sm font-semibold">{def.sectionSub ?? section}</p>
                          {def.hint && <p className="text-[11px] font-semibold text-mist">{def.hint}</p>}
                        </div>
                        {def.hasToggle && (
                          <Switch
                            checked={enabled}
                            onChange={(v) => setLine(def.key, { enabled: v })}
                            aria-label={`Toggle ${def.section} ${def.sectionSub ?? def.key}`}
                          />
                        )}
                      </div>
        ...
        {(tab === 'inventory' || tab === 'manufacturing') && (() => {
          const groups = tab === 'inventory' ? INVENTORY_GROUPS : MANUFACTURING_GROUPS
          return (
            <>
              <div className="space-y-4">
                {groups.map((group) => {
        ...
        {tab !== 'general' && tab !== 'payslips' && tab !== 'purchase' && tab !== 'inventory' && tab !== 'manufacturing' && (
