...
import { DISCOUNT_TYPES, DISCOUNT_APPLIES, computeDiscount } from '../../lib/discounts'
...
export function Discounts() {
  const app = useApp()
  const { discounts, upsertDiscount, deleteDiscount, log } = app
    ...
    <div>
      ...
      <div className="card table-wrap">
        <table className="data">
          <thead><tr><th>Code</th><th>Name</th><th>Type</th><th>Value</th><th>Applies to</th><th>Usage</th><th>Valid until</th><th>Status</th><th>ACTIONS</th></tr></thead>
          <tbody>
            {rows.map((d) => (
              <tr key={d.id}>
            ...
            {editing.value && Number(editing.value) > 0 && (
              <div className="rounded-xl border border-lime/30 bg-lime/5 p-3 text-sm">
                ...
                <p className="mt-1 text-mist">
                  A GHS 1000 order would get{' '}
                  <span className="font-semibold text-inherit">
                    {formatGhs(computeDiscount({ ...(editing as unknown as Discount), status: 'active', used: 0 }, 1000))}
                  </span>{' '}
                  off with this code.
                </p>
              </div>
