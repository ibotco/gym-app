...
export type ProductFormValues = {
  ...
  description: string
  imageName: string
  /** Final product image value: path/data URL to keep, '' = removed, undefined = none. */
  imageData?: string
  brochureName: string
  enableImei: boolean
  notForSelling: boolean
  /** Catalogue status: inactive products leave POS but stay listed. */
  active: boolean
  weight: string
  prepTime: string
  taxName: string
  sellingPriceTaxType: 'Exclusive' | 'Inclusive'
  productType: 'Single' | 'Variable' | 'Combo' | 'Service' | 'Non-Inventory'
  purchaseExc: string
  purchaseInc: string
  marginPct: string
  sellingExc: string
...
const emptyForm = (defaults?: Partial<ProductFormValues>): ProductFormValues => ({
  ...
  categoryId: '',
  locations: [],
  /** Default: products are stock-managed; turn off for the Non-inventory status. */
  manageStock: true,
  alertQuantity: '',
  description: '',
  imageName: '',
...
// Product image picker with live preview. Reads the chosen file, downscales it
// client-side (same pipeline as profile pictures) so the data URL stays small
// enough to persist with the inventory record, and previews it immediately.
const PRODUCT_IMAGE_MAX_BYTES = 5 * 1024 * 1024 // 5 MB

function ProductImagePicker({
  fileName, dataUrl, onPick, onRemove,
}: {
  fileName: string
  dataUrl?: string
  onPick: (name: string, dataUrl: string) => void
  onRemove: () => void
}) {
  const ref = useRef<HTMLInputElement>(null)
  ...
  return (
    <div>
      ...
      <div className="flex items-start gap-3">
        <div className="grid size-[72px] shrink-0 place-items-center overflow-hidden rounded-[8px] border border-[#9aa0a6] dark:border-[#49515c] bg-white dark:bg-[#14171c]">
          {preview
            ? <img src={preview} alt="Product image preview" className="size-full object-cover" />
            : <PackageOpen className="size-6 text-[#7b8794]" aria-hidden />}
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-stretch overflow-hidden rounded-[8px] border border-[#9aa0a6] dark:border-[#49515c] bg-white dark:bg-[#14171c] h-[42px]">
            ...
            <button type="button" disabled={busy} onClick={() => ref.current?.click()}
              className="h-full px-3 text-[12px] font-semibold text-white transition disabled:opacity-60"
            ...
            <input ref={ref} type="file" accept="image/*" className="hidden"
              onChange={async (e) => {
                ...
                if (file.size > PRODUCT_IMAGE_MAX_BYTES) { setError('The image must be 5 MB or smaller.'); return }
                setBusy(true)
                try {
                  const processed = await fileToAvatarDataUrl(file)
                  onPick(file.name, processed)
                } catch (err) {
          ...
          {error
            ...
            : <p className="mt-1 text-[11px] leading-snug text-slate-500 dark:text-slate-400">Max 5MB. Aspect ratio 1:1 recommended.</p>}
          {preview && (
            <button type="button" onClick={() => { setError(''); onRemove() }}
              className="mt-1 inline-flex items-center gap-1 text-[11px] font-semibold text-rose-500 hover:text-rose-600">
...
function SelectedTag({ children, onRemove }: { children: React.ReactNode; onRemove: () => void }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-[3px] bg-[#337ab7] px-2 py-0.5 text-[11px] font-semibold text-white shadow-sm max-w-full">
      ...
      {/* span with button semantics: this chip renders inside the trigger
          <button>, and nested <button> elements are invalid HTML (React
          hydration warning). stopPropagation keeps the outer trigger from
          toggling the dropdown when removing a tag. */}
      <span
        role="button"
        tabIndex={0}
        onClick={(e) => { e.stopPropagation(); onRemove() }}
        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); onRemove() } }}
        className="ml-0.5 -mr-0.5 grid size-3.5 cursor-pointer place-items-center rounded-sm text-white/80 hover:bg-white/20 hover:text-white"
        aria-label="Remove"
      >
      ...
      footer={
        <div className="flex flex-wrap items-center justify-end gap-2 w-full">
          <button type="button" onClick={() => save('close')}
            className="btn text-white bg-amber-500 hover:bg-amber-600 active:bg-amber-700">
            <Tag className="size-5" /> Save &amp; Add Selling-Price-Group Prices
          </button>
          <button type="button" onClick={() => save('close')}
            className="btn text-white bg-violet-600 hover:bg-violet-700 active:bg-violet-800">
            <Layers className="size-5" /> Save &amp; Add Opening Stock
          </button>
          <button type="button" onClick={() => save('another')}
            className="btn text-white bg-rose-600 hover:bg-rose-700 active:bg-rose-800">
            Save And Add Another
          </button>
          <button type="button" onClick={() => save('close')}
            className="btn text-white bg-[#284a72] hover:bg-[#1f3a5c] active:bg-[#172c47]">
            Save
          </button>
        </div>
      }
      ...
      <div className="product-form-modal space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-4 items-start">
            <div className="md:col-span-2 space-y-3">
              <div className="grid grid-cols-1 gap-3 items-start">
                <div>
                  <Toggle checked={v.manageStock} onChange={(val) => {
                    set('manageStock', val)
                    // Keep the stock toggle and the type option in lockstep.
                    if (!val) set('productType', 'Non-Inventory')
                    else if (v.productType === 'Non-Inventory') set('productType', 'Single')
                  }}
                  ...
                  <p className="mt-1 ml-9 text-[11px] text-slate-500 dark:text-slate-400">
                    {v.manageStock
                      ...
                      : 'Status: Non-inventory — stock is not tracked; the item is unlimited at the POS and leaves stock reports.'}
                  </p>
                </div>
                {v.manageStock && (
                  <div>
            ...
            <ProductImagePicker
              ...
              dataUrl={v.imageData}
              onPick={(name, dataUrl) => setV({ ...v, imageName: name, imageData: dataUrl })}
              onRemove={() => setV({ ...v, imageName: '', imageData: '' })}
            />
          </div>
        </SectionCard>
        ...
        <SectionCard icon={ToggleLeft} title="Additional settings">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                <FieldLabel hint hintText="Inactive products stay in the catalogue and reports but are hidden from the point-of-sale catalogue.">Status</FieldLabel>
                <Select value={v.active ? 'active' : 'inactive'} onChange={(e) => set('active', e.target.value === 'active')}>
        ...
        <SectionCard icon={Percent} title="Pricing & tax">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-3">
            ...
            <div>
              <FieldLabel required hint hintText="Single: standard product. Variable: product with variations (e.g. sizes/colours). Combo: bundle of other products. Service: non-physical service. Non-Inventory: sell unlimited — stock is not tracked.">Product Type</FieldLabel>
              <Select value={v.productType} onChange={(e) => {
                const t = e.target.value as ProductFormValues['productType']
                set('productType', t)
                // Keep the stock toggle and the type option in lockstep.
                if (t === 'Non-Inventory') set('manageStock', false)
                else if (!v.manageStock) set('manageStock', true)
              }}>
