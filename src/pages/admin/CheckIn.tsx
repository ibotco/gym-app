...
export function CheckInDesk() {
  const { members, users, branches, activeBranchId, activeCompanyId, memberships, plans, attendance, checkIn, log } = useApp()
  const { user } = useAuth()
  const toast = useToast()
  const branchOptions = visibleBranches(user, branches, activeCompanyId).filter((branch) => branch.status !== 'inactive')
  const [code, setCode] = useState('')
  const [branchId, setBranchId] = useState(activeBranchId || branchOptions[0]?.id || 'br_airport')
  useEffect(() => { if (activeBranchId && branchOptions.some((branch) => branch.id === activeBranchId)) setBranchId(activeBranchId) }, [activeBranchId, branchOptions])
