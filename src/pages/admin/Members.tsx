...
import { Plus, Download, Trash2, Pencil, KeyRound } from 'lucide-react'
import { PageHeader, Button, Input, Select, SearchInput, StatusBadge, Avatar, Modal, Field } from '../../components/ui'
import { useApp } from '../../context/AppContext'
import { useAuth } from '../../context/AuthContext'
import { useToast } from '../../context/ToastContext'
import { downloadText, toCsv, formatDate } from '../../lib/utils'
...
import type { Member, MembershipStatus, Status } from '../../types'
...
const emptyForm = (planId = 'pl_month', branchId = 'br_airport'): MemberForm => ({
  ...
  emName: '', emPhone: '', emRel: '', heightCm: '170', weightKg: '70', trainerId: '',
})
    ...
    <div>
        ...
        actions={
          <>
            <Button variant="outline" onClick={() => {
              downloadText('members.csv', toCsv(rows.map((r) => ({
      ...
      <div className="card table-wrap">
        <table className="data">
          <thead>
            <tr>
              <th>Member</th><th>Plan</th><th>Status</th><th>Tags</th><th>Joined</th><th>Renews</th><th></th>
            </tr>
          </thead>
          <tbody>
            {rows.map(({ m, u, ms, pl }) => (
              <tr key={m.id}>
                ...
                <td>{pl?.name}</td>
                <td><StatusBadge status={ms?.status || u?.status || 'active'} /></td>
                <td className="max-w-[180px]">
                  <div className="flex flex-wrap gap-1">{m.tags.map((t) => <span key={t} className="chip bg-white/5 text-[10px]">{t}</span>)}</div>
                </td>
                <td className="text-mist">{formatDate(m.joinDate)}</td>
                <td className="text-mist">{ms ? formatDate(ms.endDate) : '—'}</td>
