...
export function Members() {
  ...
  const columns: Column<Row>[] = [
    ...
    { key: 'branch', header: 'Branch', sortValue: (r) => branches.find((branch) => branch.id === (r.m.branchId || r.u?.branchId || r.ms?.branchId))?.name || '', render: (r) => branches.find((branch) => branch.id === (r.m.branchId || r.u?.branchId || r.ms?.branchId))?.name || '—' },
    {
  ...
  const openCreate = () => {
    setEditingId(null)
    setForm(emptyForm(plans[0]?.id, app.activeBranchId || branchOptions[0]?.id || 'br_airport'))
    setErr('')
    setOpen(true)
  }
  ...
  const openEdit = (m: Member) => {
    ...
    setForm({
      ...
      branchId: m.branchId || u?.branchId || ms?.branchId || app.activeBranchId || branchOptions[0]?.id || '',
  ...
  const validate = () => {
    ...
    if (!form.branchId || !branchOptions.some((branch) => branch.id === form.branchId)) return 'Select a permitted home club.'
        ...
        actions={
          <>
            <ExportButtons
              filename="members"
              rows={rows.map((r) => ({
                name: r.u?.name, email: r.u?.email, plan: r.pl?.name, branch: branches.find((branch) => branch.id === (r.m.branchId || r.u?.branchId || r.ms?.branchId))?.name || '', status: r.ms?.status, join: r.m.joinDate,
              }))}
