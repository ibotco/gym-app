...
const blank = (defaultCategory = ''): FormState => ({
  name: '', email: '', phone: '+233 ', company: '', address: '', category: defaultCategory, status: 'active', notes: '',
  customFields: {},
})
...
export function Customers() {
  const app = useApp()
  const { customers, customerCategories, upsertCustomer, deleteCustomer, log } = app
    ...
    { key: 'joined', header: 'Added', sortValue: (c) => c.createdAt, render: (c) => <span className="text-mist">{formatDate(c.createdAt)}</span> },
    {
      ...
      render: (c) => (
        <span className="whitespace-nowrap">
          {canManage && (
            <PortalAccess
              subject={{ id: c.id, name: c.name, email: c.email, phone: c.phone, userId: c.userId }}
              role="customer"
              entity="Customer"
  ...
  const openNew = () => setEditing(blank(customerCategories[0] || ''))
  const openEdit = (c: Customer) => setEditing({
    id: c.id, name: c.name, email: c.email, phone: c.phone, company: c.company || '',
    address: c.address || '', category: c.category, status: c.status, notes: c.notes || '',
  ...
  const save = () => {
    ...
    const rec: Customer = {
      ...
      totalSpent: isNew ? 0 : (customers.find((c) => c.id === editing.id)?.totalSpent || 0),
      createdAt: isNew ? new Date().toISOString() : (customers.find((c) => c.id === editing.id)?.createdAt || new Date().toISOString()),
    }
    upsertCustomer(rec)
  ...
  const exportRows = rows.map((c) => ({
    ...
    category: c.category, status: c.status, totalSpent: c.totalSpent, added: c.createdAt,
  }))
