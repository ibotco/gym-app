...
import { PageHeader, Button, Badge, StatusBadge, Field, Input, Textarea, Avatar, Modal, Select, DatePicker } from '../../components/ui'
...
export function MemberDetail() {
  ...
  const branchOptions = visibleBranches(user, app.branches, app.activeCompanyId).filter((branch) => branch.status !== 'inactive')
    ...
    <div>
            ...
            {canEdit && (
              <Button variant="outline" onClick={() => {
                setForm({
                  ...
                  branchId: m.branchId || u.branchId || ms?.branchId || app.activeBranchId || branchOptions[0]?.id || '', planId: ms?.planId || m.planId, trainerId: m.trainerId || '',
        ...
        <div className="space-y-4">
          <div className="card flex items-center gap-4 p-5">
            ...
            <div>
              ...
              <p className="text-sm text-mist">Home club: {app.branches.find((b) => b.id === (m.branchId || u.branchId || ms?.branchId))?.name || 'Unassigned'}</p>
            </div>
          </div>
      ...
      <Modal open={editOpen} onClose={() => setEditOpen(false)} title="Edit member" wide>
        ...
        <Button className="mt-4 w-full" onClick={() => {
          ...
          if (!form.branchId || !branchOptions.some((branch) => branch.id === form.branchId)) { setErr('Select a permitted home branch.'); return }
