...
import { ShoppingCart, Plus, Minus, Receipt, Banknote, CreditCard, Smartphone, X, ChevronDown, Percent, Tags, Layers, Check as CheckIcon, ArrowLeft, Trash2, User, MapPin, CalendarDays, Maximize2, Minimize2, PackageSearch, ScanLine } from 'lucide-react'
...
export function PointOfSale({ screen }: { screen?: number }) {
  ...
  /** Enter on the scan field adds the single matching product — same addToCart rules. */
  const onScanKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key !== 'Enter') return
    event.preventDefault()
    const needle = q.trim().toLowerCase()
    if (!needle) return
    const exact = products.find((i) => i.sku.toLowerCase() === needle)
    const target = exact || (products.length === 1 ? products[0] : undefined)
    if (!target) {
      ...
      {screen !== 2 && (
        ...
        <div className="grid min-h-0 flex-1 gap-4 px-3 pb-4 pt-3 md:px-4 lg:grid-cols-[1.5fr_1fr]">
          ...
          <div className="card flex min-h-0 flex-col p-4">
            ...
            <Button className="mt-3 w-full shrink-0" size="lg" disabled={!cart.length} onClick={requestCompleteSale}>
      ...
      {screen === 2 && (
        ...
        <div className="grid min-h-0 flex-1 gap-3 px-3 pb-3 pt-3 md:px-4 lg:grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)] lg:gap-4">
          ...
          <section aria-label="Current transaction" className="card flex min-h-0 flex-col overflow-hidden p-0">
            {/* Customer + scan row */}
            <div className="grid shrink-0 gap-2 border-b border-line p-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)]">
              <div className="flex items-center gap-2">
              ...
              <div className="grid gap-2">
                <div className="relative">
                  <ScanLine className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-mist" aria-hidden />
                  <input
                    ref={scanRef}
                    value={q}
                    ...
                    autoComplete="off"
                    spellCheck={false}
                    aria-label="Enter product name, SKU or scan bar code"
            ...
            <div className="shrink-0 border-t border-line">
              ...
              >
                <span className="flex items-center gap-2 text-sm font-semibold"><Percent className="size-4 text-lime" aria-hidden /> Discount &amp; tax <span className="text-xs font-normal text-mist">Optional</span></span>
                <span className="flex items-center gap-2 text-xs font-semibold text-mist">
                  {appliedDiscount > 0
                    ? `${formatGhsExact(appliedDiscount)} discount`
                    : orderTax > 0
            ...
            <dl className="grid shrink-0 grid-cols-2 border-t border-line text-center sm:grid-cols-5">
              {([
                { label: 'Items', value: cartItemCount.toFixed(2), tone: '' },
                { label: 'Subtotal', value: formatGhsExact(subtotal), tone: '' },
                { label: 'Discount (−)', value: formatGhsExact(appliedDiscount), tone: 'text-rose-600 dark:text-rose-300' },
                { label: 'Order tax (+)', value: formatGhsExact(orderTax), tone: 'text-emerald-600 dark:text-emerald-300' },
              ]).map((cell) => (
            ...
            <div className="flex shrink-0 flex-wrap items-center gap-2 border-t border-line p-3">
              ...
              <Button size="lg" className="ml-auto min-w-[13rem] flex-1 sm:flex-none" disabled={!cart.length} onClick={requestCompleteSale}>
                  ...
                  >
                    ...
                    <div className="flex flex-1 flex-col gap-0.5 p-2">
                      ...
                      <p className={cn('text-[10px]', out ? 'text-ember' : st === 'low' ? 'text-amber-500' : 'text-mist')}>
                        {out ? 'Out of stock' : `${i.quantity} ${i.unit} in stock`}
                      </p>
                    </div>
                  </button>
                )
