...
export function StaffAttendance() {
  const app = useApp()
  const { staffAttendance, staff, users, branches, activeBranchId, activeCompanyId, upsertStaffAttendance, deleteStaffAttendance, log } = app
  ...
  const openNew = () => setEditing({ staffUserId: staff[0]?.userId || '', date: today, checkIn: '08:00', checkOut: '17:00', status: 'present', branchId: activeBranchId || branchOptions[0]?.id || '', notes: '' })
  ...
  const save = () => {
    ...
    const assignedStaff = staff.find((record) => record.userId === editing.staffUserId)
    if (assignedStaff?.branchId && assignedStaff.branchId !== editing.branchId) { toast.error('Attendance branch must match the employee home branch.'); return }
