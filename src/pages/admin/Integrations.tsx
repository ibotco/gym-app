...
import { PAYAZA_ID, testPayazaConnection } from '../../lib/payaza'
import {
  FLUTTERWAVE_ID, STRIPE_ID, PAYPAL_ID, HUBTEL_ID,
  testFlutterwaveConnection, testStripeConnection, testPaypalConnection, testHubtelConnection,
} from '../../lib/gateways'
...
export function Integrations({ embedded }: { embedded?: boolean }) {
  ...
  const runTest = async (kind: 'test' | 'health') => {
    ...
    const result = preview.id === PAYSTACK_ID
      ...
      : preview.id === PAYAZA_ID
        ? await testPayazaConnection(preview)
        : preview.id === FLUTTERWAVE_ID
          ? await testFlutterwaveConnection(preview)
          : preview.id === STRIPE_ID
            ? await testStripeConnection(preview)
            : preview.id === PAYPAL_ID
              ? await testPaypalConnection(preview)
              : preview.id === HUBTEL_ID
                ? await testHubtelConnection(preview)
          ...
          <div>
            ...
            {tab === 'setup' && (
              <div className="mt-4 space-y-4">
                ...
                {current.id === PAYAZA_ID && (
                  <PayazaExtras draft={draft} setDraft={setDraft} />
                )}
                {(current.id === FLUTTERWAVE_ID || current.id === STRIPE_ID || current.id === PAYPAL_ID || current.id === HUBTEL_ID) && (
                  <GatewayExtras
                    draft={draft}
                    setDraft={setDraft}
                    provider={current.name}
                    showChannel={current.id === HUBTEL_ID}
                  />
                )}
...
function PayazaExtras({
  ...
  return (
    <div className="space-y-3 rounded-xl border border-white/10 p-3">
      <p className="text-sm font-semibold">Payaza checkout</p>
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Currency">
          <Select value={extra.currency || 'GHS'} onChange={(e) => setExtra('currency', e.target.value)}>
            <option value="GHS">GHS — Ghana cedi</option>
      ...
      <p className="text-xs text-mist">
        Payaza uses its hosted Payment Page — members are redirected to a secure checkout for card, bank transfer, and mobile money. Paste your public API key (merchant key) in the API key field above.
      </p>
      <label className="flex items-center gap-2 text-sm">
        <input
          type="checkbox"
          ...
          onChange={(e) => setExtra('autoSettle', e.target.checked ? 'true' : 'false')}
        />
        Auto-mark invoice paid after a confirmed Payaza payment
      </label>
      <label className="flex items-center gap-2 text-sm">
        <input
          type="checkbox"
          ...
          onChange={(e) => setExtra('allowDemo', e.target.checked ? 'true' : 'false')}
        />
        Allow demo checkout when no key is set (no real money)
      </label>
    </div>
  )
}
...
function GatewayExtras({
  ...
  return (
    <div className="space-y-3 rounded-xl border border-white/10 p-3">
      <p className="text-sm font-semibold">{provider} checkout</p>
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Currency">
          <Select value={extra.currency || 'GHS'} onChange={(e) => setExtra('currency', e.target.value)}>
            <option value="GHS">GHS — Ghana cedi</option>
        ...
        {showChannel && (
          <Field label="Mobile money channel">
            <Select value={extra.channel || 'mtn-gh'} onChange={(e) => setExtra('channel', e.target.value)}>
      ...
      <label className="flex items-center gap-2 text-sm">
        <input
          ...
          onChange={(e) => setExtra('autoSettle', e.target.checked ? 'true' : 'false')}
        />
        Auto-mark invoice paid after a confirmed {provider} payment
      </label>
      <label className="flex items-center gap-2 text-sm">
        <input
          type="checkbox"
