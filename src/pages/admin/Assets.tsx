...
import { Plus, Pencil, Trash2, Printer, Boxes, Wrench, Tag, MapPin, User, SlidersHorizontal, Info } from 'lucide-react'
...
const blank = (tag: string, category = ''): FormState => ({
  ...
  accumulatedDepreciationAccountId: 'ac_5', warrantyExpiry: '', notes: '',
})
...
export function Assets() {
  const app = useApp()
  const { assets, upsertAsset, deleteAsset, log, company, branches, assetCategories, assetConditions, accounts } = app
  ...
  const save = () => {
    ...
    if (salvageValue < 0 || salvageValue >= purchaseCost) { toast.error('Salvage value must be between 0 and the purchase price.'); return }
    const rec: Asset = {
