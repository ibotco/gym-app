...
export function SaleEditorPage({ sale, onClose }: SaleEditorPageProps) {
  const app = useApp()
  const { inventory, members, users, branches, accounts, branchSettings, activeBranchId, recordSale, updateSale, log } = app
  ...
  const orderDiscount = Math.min(netSubtotal, discountType === 'percentage' ? netSubtotal * Math.max(0, discountAmount) / 100 : Math.max(0, discountAmount))
        ...
        <div className="overflow-x-auto rounded-xl border border-line">
          <table className="w-full min-w-[800px] text-sm">
            ...
            <tbody className="divide-y divide-line">
              {lines.length === 0 && (
                <tr><td colSpan={7} className="px-4 py-12 text-center"><Package className="mx-auto size-8 text-muted" /><p className="mt-2 text-sm font-semibold text-zinc-900 dark:text-zinc-100">No products added</p><p className="mt-1 text-xs text-muted">Search for a product above to start this sale.</p></td></tr>
              )}
            ...
            <tfoot className="border-t border-line bg-zinc-50 dark:bg-white/[0.025]">
              <tr><td colSpan={5} className="px-3 py-3 text-right text-xs font-bold uppercase tracking-wide text-muted">Items total</td><td className="px-3 py-3 text-right font-black text-zinc-900 dark:text-zinc-100">{formatGhsExact(grossSubtotal)}</td><td /></tr>
            </tfoot>
          </table>
        </div>
      ...
      >
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Field label={discountType === 'percentage' ? 'Discount (%)' : 'Discount amount'}>
              <Input type="number" min="0" step="0.01" value={discountAmount} onChange={(event) => setDiscountAmount(Math.max(0, numberValue(event.target.value)))} />
            </Field>
            ...
            <div className="rounded-xl border border-line bg-zinc-50 p-3 dark:bg-white/[0.025]">
              ...
              <div className="mt-3 space-y-2 border-t border-line pt-3">
                ...
                <div className="flex items-center justify-between gap-3 text-sm"><span className="min-w-0 text-muted">Order discount</span><span className="shrink-0 font-semibold tabular-nums text-zinc-900 dark:text-zinc-100">{formatGhsExact(orderDiscount)}</span></div>
