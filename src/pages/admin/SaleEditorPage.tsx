...
export function SaleEditorPage({ sale, onClose }: SaleEditorPageProps) {
  ...
  const codeShortfall = selectedDiscount && codeBlock && selectedDiscount.minSpend && netSubtotal < selectedDiscount.minSpend
    ...
    : 0
  const orderDiscount = codeDiscountValue ?? Math.min(netSubtotal, discountType === 'percentage' ? netSubtotal * Math.max(0, discountAmount) / 100 : Math.max(0, discountAmount))
      ...
      >
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Field label={discountType === 'percentage' ? 'Discount (%)' : 'Discount amount'}>
              <Input type="number" min="0" step="0.01" disabled={!!selectedDiscount} value={discountAmount} onChange={(event) => setDiscountAmount(Math.max(0, numberValue(event.target.value)))} />
            </Field>
